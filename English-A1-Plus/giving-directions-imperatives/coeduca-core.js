...
(function (global) {
  ...
  function renderColorido(ctx) {
    ...
    if (state.extraPoints > 0) {
      ...
      doc.text('+' + state.extraPoints.toFixed(1), M + contentW - 28, y + 16);
    }
    y += 38;
  ...
  function renderPills(ctx, y, opts) {
    ...
    const hasExtra = ctx.state.extraPoints > 0;
    ...
    if (hasExtra) {
      ...
      pills.push({ label: 'PUNTOS EXTRA', value: '+' + ctx.state.extraPoints.toFixed(1), highlight: true });
    } else {
      pills.push({ label: 'NOTA BASE', value: ctx.score.baseGrade + ' / 10' });
    }
  ...
  function renderPillsDashed(ctx, y, opts) {
    ...
    if (hasExtra) {
      pills.push({ label: 'nota base', value: ctx.score.baseGrade + ' / 10' });
      pills.push({ label: 'extra', value: '+' + ctx.state.extraPoints.toFixed(1) });
    } else {
      pills.push({ label: 'nota base', value: ctx.score.baseGrade + ' / 10' });
    }
    pills.forEach(function (pill, i) {
