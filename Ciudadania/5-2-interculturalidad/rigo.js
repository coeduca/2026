/**
 * RIGO THE CHAMELEON v2 - Mascota oficial del ecosistema de inglés
 * Estilo: Kawaii realista (silueta de camaleón con cresta, cola enrollada, 4 patas)
 *
 * Uso:  <script src="rigo.js"></script>
 *       <rigo-mascot grade="Noveno"></rigo-mascot>
 *       (el atributo grade filtra los hints personalizados)
 *
 * API pública:
 *   rigo.welcome()        -> saludo inicial antes del NIE
 *   rigo.loginSuccess(n)  -> celebración al ingresar
 *   rigo.setGrade(grade)  -> filtra hints al grado actual
 *   rigo.inviteGame()     -> invita al juego final
 *   rigo.cheer()          -> festeja una respuesta correcta
 *   rigo.comfort()        -> consuela tras un error
 *   rigo.say(texto, ms)   -> mensaje libre
 *   rigo.setEmotion(e)    -> cambia emoción manualmente
 *
 * Eventos emitidos:
 *   rigo-hint-requested   -> cuando el estudiante toca a Rigo pidiendo ayuda
 */
(function () {
  'use strict';

  const EMOTIONS = [
    'neutral', 'welcome', 'happy', 'confused', 'thinking',
    'sneaky', 'sad', 'excited', 'game', 'love', 'angry',
    'dizzy', 'musical', 'elder', 'easterEgg'
  ];

  // MENSAJES: cada uno puede tener:
  //   - text:    el mensaje (obligatorio)
  //   - grade:   null = todos los grados; array = solo esos grados
  //   - emotion: emoción forzada al mostrar el mensaje (opcional)
  //              Si no se define, usa la emoción por defecto del contexto.
  const RANDOM_MESSAGES = [
    { text: "I missed English", grade: null, emotion: 'confused' },
    { text: "El pasado ya pasó", grade: null, emotion: 'happy' },
    { text: "Toma nota", grade: null, emotion: 'sneaky' },
    { text: "No me asusta ni la siguanaba", grade: null, emotion: 'dizzy' },
    { text: "Me gusta el sushi", grade: null, emotion: 'happy' },
    { text: "Hace mucho calor", grade: null, emotion: 'sad' },
    { text: "Peliculas? Que tema es este?", grade: null, emotion: 'thinking' },
    { text: "No puedo hacer milagros", grade: null, emotion: 'angry' },
    { text: "Aqui como que huele mal", grade: null, emotion: 'dizzy' },
    { text: "Guíate con el libro", grade: null, emotion: 'happy' },
    { text: "Puedes hacer mucho con tu conocimiento", grade: null, emotion: 'happy' },
    { text: "Avanza sin mirar atrás", grade: null, emotion: 'happy' },
    { text: "Dime con quien andas y te diré quien eres", grade: null, emotion: 'happy' },
    { text: "Eres muy inteligente", grade: null, emotion: 'happy' },
    { text: "Lo haces como un sabedor", grade: null, emotion: 'happy' },
    { text: "Solo tu puedes hacerlo", grade: null, emotion: 'happy' },
    { text: "Trabajar en equipo puede hacer el trabajo más fácil", grade: null, emotion: 'happy' },
    { text: "Inglés es fácil", grade: null, emotion: 'sneaky' },
    { text: "Creo en ti", grade: null, emotion: 'happy' },
    { text: "Practicar inglés no hace daño.", grade: null, emotion: 'happy' },
    { text: "Yo sí creo en ti.", grade: null, emotion: 'happy' },
    { text: "\"How many?\" es \"¿Cuantos?\"", grade: null, emotion: 'happy' },
    { text: "Puedes terminarlo a tu ritmo", grade: null, emotion: 'happy' },
    { text: "Tranqui, no hay prisa.", grade: null, emotion: 'happy' },
    { text: "继续工作", grade: null, emotion: 'happy' },
    { text: "Keep it cool", grade: null, emotion: 'happy' },
    { text: "¡Abueloo tlalooc! Ayudanos", grade: null, emotion: 'happy' },
    { text: "Vamos, vamos", grade: null, emotion: 'happy' },
    { text: "Let\'s go", grade: null, emotion: 'happy' },
    { text: "我是里戈", grade: null, emotion: 'happy' },
    { text: "No soy tu compañero, soy tu compañere", grade: null, emotion: 'angry' },
    { text: "Aquí ya no hay postes", grade: null, emotion: 'dizzy' },
    { text: "Entra al cuarto, subele el volumen al radio...", grade: null, emotion: 'musical' },
    { text: "I\'m the most beautiful person in the classroom", grade: null, emotion: 'happy' },
    { text: "Que hermoso es estudiar", grade: null, emotion: 'excited' },
    { text: "What a beautiful thing is to study", grade: null, emotion: 'excited' },
    { text: "Ok, pero mi novia", grade: null, emotion: 'love' },
    { text: "Soy el más hermosos del salón", grade: null, emotion: 'happy' },
    { text: "Todo me recuerda a ella", grade: null, emotion: 'sad' },
    { text: "Pienso mucho", grade: null, emotion: 'thinking' },
    { text: "Let me belooong", grade: null, emotion: 'musical' },
    { text: "Tu tranqui", grade: null, emotion: 'happy' },
    { text: "Lo haces bien", grade: null, emotion: 'happy' },
    { text: "Estudia mucho", grade: null, emotion: 'happy' },
    { text: "Cree en ti", grade: null, emotion: 'happy' },
    { text: "¿Cómo estás?", grade: null, emotion: 'happy' },
    { text: "Creo en ti", grade: null, emotion: 'happy' },
    { text: "Estoy feliz de verte trabajando", grade: null, emotion: 'happy' },
    { text: "Welcome back", grade: null, emotion: 'happy' },
    { text: "Studying hard", grade: null, emotion: 'happy' },
    { text: "Sigue como vas", grade: null, emotion: 'happy' },
    { text: "Hello", grade: null, emotion: 'happy' },
    { text: "Hola", grade: null, emotion: 'happy' },
    { text: "No te canses de intentar", grade: null, emotion: 'happy' },
    { text: "Sigue así", grade: null, emotion: 'happy' },
    { text: "Todo está en la mente", grade: null, emotion: 'happy' },
    { text: "¡Cada derecho que tenemos hoy tuvo una historia detrás!", grade: null, emotion: 'happy' },
    { text: "¿Sabías que durante mucho tiempo muchas mujeres no podían votar?", grade: null, emotion: 'happy' },
    { text: "Las mujeres también tienen voz", grade: null, emotion: 'happy' },
    { text: "It\'s all in your head", grade: null, emotion: 'happy' },
    { text: "Todo lo que necesitas es amor", grade: null, emotion: 'happy' },
    { text: "Hazlo con paciencia", grade: null, emotion: 'happy' },
    { text: "Demuestra lo que sabes", grade: null, emotion: 'happy' },
    { text: "Cuando sea grande quiero ser bombero", grade: null, emotion: 'happy' },
    { text: "Ánimos, ánimos", grade: null, emotion: 'happy' },
    { text: "Vamos, primer año nos espera", grade: ["Noveno"], emotion: 'happy' },
    { text: "Vamos, segundo año nos espera", grade: ["Primer Año"], emotion: 'happy' },
    { text: "Vamos, la universidad nos espera", grade: ["Segundo Año"], emotion: 'happy' },
    { text: "Podés hacerlo", grade: null, emotion: 'happy' },
    { text: "It\'s all in your head", grade: null, emotion: 'happy' },
    { text: "Estudia y ganarás", grade: null, emotion: 'happy' },
    { text: "Una preposición indica el lugar donde algo está posicionando", grade: null, emotion: 'sneaky' },
    { text: "Vamos, sigue así", grade: null, emotion: 'happy' },
    { text: "Todo es mejor cuando estudias", grade: null, emotion: 'happy' },
    { text: "Todo está en la mente", grade: null, emotion: 'happy' },
    { text: "No puedo ayudarte", grade: null, emotion: 'happy' },
    { text: "¿Estas bien?", grade: null, emotion: 'confused' },
    { text: "Nos volvemos a ver", grade: null, emotion: 'happy' },
    { text: "Espero que te encuentres bien", grade: null, emotion: 'happy' },
    { text: "Todo parece estar bien por aquí", grade: null, emotion: 'happy' },
    { text: "2 + 2 = 4", grade: null, emotion: 'sneaky' },
    { text: "Esto es fácil de hacer", grade: null, emotion: 'happy' },
    { text: "Hello", grade: null, emotion: 'happy' },
    { text: "Your dreams may come true", grade: null, emotion: 'happy' },
    { text: "Todo está en la actitud", grade: null, emotion: 'happy' },
    { text: "Son solo 6 ejercicios", grade: null, emotion: 'happy' },
    { text: "Vamos, vamos", grade: null, emotion: 'happy' },
    { text: "Creo que hoy va a llover", grade: null, emotion: 'happy' },
    { text: "It\'s good to be here again", grade: null, emotion: 'happy' },
    { text: "How much is it?", grade: null, emotion: 'happy' },
    { text: "¿Vas a trabajar o a perder el tiempo?", grade: null, emotion: 'neutral' },
    { text: "Vamos, sigue trabajando", grade: null, emotion: 'neutral' },
    { text: "¿Como has estado?", grade: null, emotion: 'happy' },
    { text: "Hoy no hace tanto frío", grade: null, emotion: 'excited' },
    { text: "Hello Hello", grade: null, emotion: 'happy' },
    { text: "¿Todo bien?", grade: null, emotion: 'excited' },
    { text: "Relajate", grade: null, emotion: 'happy' },
    { text: "Hazlo con calma", grade: null, emotion: 'happy' },
    { text: "Hello there", grade: null, emotion: 'happy' },
    { text: "Hey you", grade: null, emotion: 'happy' },
    { text: "¿Hoy si terminarás?", grade: null, emotion: 'excited' },
    { text: "Sigue adelante", grade: null, emotion: 'happy' },
    { text: "Hazlo, tu puedes", grade: null, emotion: 'happy' },
    { text: "Great day", grade: null, emotion: 'happy' },
    { text: "Listen! Keep it cool", grade: null, emotion: 'happy' },
    { text: "We are ready", grade: null, emotion: 'happy' },
    { text: "¿Estás bien?", grade: null, emotion: 'thinking' },
    { text: "Que miedo no saber inglés", grade: null, emotion: 'sad' },
    { text: "Mi familia es muy grande", grade: null, emotion: 'sneaky' },
    { text: "Tu puedes hacerlo", grade: null, emotion: 'happy' },
    { text: "You can do it!", grade: null, emotion: 'happy' },
    { text: "Nos vemos otra vez", grade: null, emotion: 'happy' },
    { text: "Y soporta", grade: null, emotion: 'sneaky' },
    { text: "Al fin llegamos a la unidad 3", grade: null, emotion: 'elder' },
    { text: "It\'s time to practice", grade: null, emotion: 'excited' },
    { text: "Sometimes it is good to study", grade: null, emotion: 'sneaky' },
    { text: "Hello hello", grade: null, emotion: 'welcome' },
    { text: "Debes ser imparable", grade: null, emotion: 'sneaky' },
    { text: "A mi me gusta la comida, o mejor dicho la food", grade: null, emotion: 'love' },
    { text: "do it", grade: null, emotion: 'sneaky' },
    { text: "Todo va bien", grade: null, emotion: 'happy' },
    { text: "Este tema está fácil", grade: null, emotion: 'sneaky' },
    { text: "Bienvenido a la unidad 3", grade: null, emotion: 'sneaky' },
    { text: "Welcome my friend", grade: null, emotion: 'neutral' },
    { text: "Nos vemos de nuevo", grade: null, emotion: 'happy' },
    { text: "Sigue adelante", grade: null, emotion: 'sneaky' },
    { text: "Un nuevo día, nuevas oportunidades", grade: null, emotion: 'neutral' },
    { text: "Hola una vez más", grade: null, emotion: 'welcome' },
    { text: "Me haces sentir orgulloso", grade: null, emotion: 'happy' },
    { text: "Manten la calma", grade: null, emotion: 'happy' },
    { text: "¿Todo bien?", grade: null, emotion: 'neutral' },
    { text: "Pongan cumbia", grade: null, emotion: 'sneaky' },
    { text: "Estamos para ayudar", grade: null, emotion: 'elder' },
    { text: "Hola", grade: null, emotion: 'welcome' },
    { text: "Terminarás rápido", grade: null, emotion: 'excited' },
    { text: "¿Todo va bien?", grade: null, emotion: 'sneaky' },
    { text: "Sigue así", grade: null, emotion: 'happy' },
    { text: "Es fácil ¿verdad?", grade: null, emotion: 'happy' },
    { text: "No olvides subirlo a Classroom", grade: null, emotion: 'thinking' },
    { text: "Six Seven", grade: ["Octavo", "Séptimo"], emotion: 'game' },
    { text: "Raios", grade: null, emotion: 'dizzy' },
    { text: "Con calma 🎶", grade: null, emotion: 'musical' },
    { text: "Compremos un ventilador entre todos", grade: null, emotion: 'excited' },
    { text: "Ya casi terminas... Las ganas", grade: null, emotion: 'thinking' },
    { text: "Let\'s go!", grade: null, emotion: 'neutral' },
    { text: "El inglés no es tan difícil", grade: null, emotion: 'sneaky' },
    { text: "Hace calor por aquí", grade: null, emotion: 'confused' },
    { text: "Hola my friend", grade: null, emotion: 'happy' },
    { text: "Tranqui", grade: null, emotion: 'thinking' },
    { text: "Respira y continúa", grade: null, emotion: 'neutral' },
    { text: "Learning is cool", grade: null, emotion: 'happy' },
    { text: "You\'re doing great!", grade: null, emotion: 'excited' },
    { text: "¡No te rindas!", grade: null, emotion: 'excited' },
    { text: "I believe in you!", grade: null, emotion: 'love' },
    { text: "Practice makes perfect", grade: null, emotion: 'thinking' },
    { text: "Keep going, friend!", grade: null, emotion: 'happy' },
    { text: "¡Tú puedes!", grade: null, emotion: 'excited' },
    { text: "Vamos por ese 10!", grade: null, emotion: 'love' },
    { text: "Quizz time!", grade: null, emotion: 'happy' }
  ];

  // HINTS: si grade es null aparecen para todos los grados.
  //        si grade es un array, solo aparecen para esos grados.
  //        emotion fuerza una emoción específica al mostrar el hint.
  // Emociones disponibles: neutral, welcome, happy, confused, thinking,
  //                        sneaky, sad, excited, game, love, angry,
  //                        dizzy, musical, elder
  // Grados válidos: "Séptimo", "Octavo", "Noveno", "Primer Año", "Segundo Año", "Prueba"
  const HINT_MESSAGES = [
    { text: "Ya te vi en los chismosos TV", grade: ["Segundo Año"], emotion: 'dizzy' },
    { text: "Se mejor persona cada día", grade: null, emotion: 'happy' },
    { text: "La vida no se siente igual sin ti", grade: null, emotion: 'sad' },
    { text: "Jamás te rindas", grade: null, emotion: 'neutral' },
    { text: "Odio decirlo, pero no me sé esa respuesta", grade: null, emotion: 'sad' },
    { text: "Solo respira y piensa", grade: null, emotion: 'sneaky' },
    { text: "Estamos avanzando", grade: null, emotion: 'love' },
    { text: "No quiero que llueva", grade: null, emotion: 'dizzy' },
    { text: "Extraño cuando estaba en forma", grade: null, emotion: 'neutral' },
    { text: "Podría comerme una ballena, tengo hambre", grade: null, emotion: 'happy' },
    { text: "Tu amiga necesita una pastilla de menta", grade: null, emotion: 'dizzy' },
    { text: "Dame tu cosita", grade: null, emotion: 'musical' },
    { text: "Deja de verme", grade: null, emotion: 'angry' },
    { text: "El amor está en el aire", grade: null, emotion: 'love' },
    { text: "Lavaría mi ropa si usara ropa", grade: null, emotion: 'happy' },
    { text: "El Holocausto fue un plan terrible", grade: null, emotion: 'sad' },
    { text: "Recordar la historia es precaución", grade: null, emotion: 'sneaky' },
    { text: "Todos podemos aportar algo a la clase", grade: null, emotion: 'happy' },
    { text: "Hoy hace calor", grade: null, emotion: 'sad' },
    { text: "Tenemos mucho que aprender de este tema", grade: null, emotion: 'love' },
    { text: "¿Ya viste el precio de las papas?", grade: null, emotion: 'confused' },
    { text: "Todos aprendemos de diferente manera", grade: null, emotion: 'happy' },
    { text: "Oh my god!", grade: null, emotion: 'excited' },
    { text: "Mi primera chamba", grade: null, emotion: 'musical' },
    { text: "Necesitamos un teléfono público en la escuela", grade: null, emotion: 'sad' },
    { text: "Si terminas después jugamos mica", grade: null, emotion: 'sneaky' },
    { text: "Todo bien mientras hagas las tareas", grade: null, emotion: 'happy' },
    { text: "Quiero comer una ensalada rusa", grade: null, emotion: 'confused' },
    { text: "No hay que llorar! Que la vida es un carnaval...", grade: null, emotion: 'musical' },
    { text: "En boca cerrada... es más difícil comer pupusas.", grade: null, emotion: 'sneaky' },
    { text: "Si la vida te da limones, has la tarea.", grade: null, emotion: 'thinking' },
    { text: "Mi abogado me recomendó no terminar esta frase", grade: null, emotion: 'dizzy' },
    { text: "El Wi-Fi siente cuando tienes prisa", grade: null, emotion: 'neutral' },
    { text: "¿Por qué el calendario estaba triste? Porque sus días estaban contados.", grade: null, emotion: 'happy' },
    { text: "📎 Este mensaje no resuelve ninguno de tus problemas", grade: null, emotion: 'neutral' },
    { text: "Ñoñoooooooo", grade: null, emotion: 'sad' },
    { text: "你敢碰我试试", grade: null, emotion: 'angry' },
    { text: "Saaaluuudeeeeemos la paaaatria orgullooosos", grade: null, emotion: 'musical' },
    { text: "Sci-fi es la abreviación de Science Fiction o sea Ciencia Ficción", grade: null, emotion: 'sneaky' },
    { text: "La curiosidad mató al camaleón", grade: null, emotion: 'elder' },
    { text: "Las cucarachas de mi casa retumban cuando pongo perreo", grade: null, emotion: 'excited' },
    { text: "Todos nacimos super estrellas", grade: null, emotion: 'excited' },
    { text: "Camarón que se duerme me lo como porque tengo hambre", grade: null, emotion: 'happy' },
    { text: "Uy llamen a la policía porque me vas a matar de la risa", grade: null, emotion: 'neutral' },
    { text: "¿Que hace una abeja en el gym? Zum-ba", grade: null, emotion: 'confused' },
    { text: "Go Go Go", grade: null, emotion: 'neutral' },
    { text: "Se que quieres volver, pero otra esta a mi lado", grade: null, emotion: 'love' },
    { text: "Uno de tus compañeros no se ha bañado", grade: null, emotion: 'dizzy' },
    { text: "Te daré un consejo: estudia y brillarás en todas partes.", grade: null, emotion: 'sneaky' },
    { text: "¿Como puedes sacar mala nota incluso con mi ayuda?", grade: null, emotion: 'thinking' },
    { text: "Piensa con la mente no con el corazón", grade: null, emotion: 'neutral' },
    { text: "Escoger a los amigos correctos también te acerca al éxito", grade: null, emotion: 'happy' },
    { text: "¿Y si yo en verdad fuera una rana, una princesa me besaria?", grade: null, emotion: 'confused' },
    { text: "Cuentame el chisme completo", grade: null, emotion: 'angry' },
    { text: "Que asco los rellenos de pacaya", grade: null, emotion: 'dizzy' },
    { text: "De hecho soy algo famoso", grade: null, emotion: 'musical' },
    { text: "En un universo alterno fuimos felices", grade: null, emotion: 'sad' },
    { text: "Todos estamos bien en inglés", grade: null, emotion: 'happy' },
    { text: "Quiero una mascota pero yo soy la mascota", grade: null, emotion: 'sad' },
    { text: "Traka", grade: null, emotion: 'sneaky' },
    { text: "Y dale maraca, maraca, maraca y dale...", grade: null, emotion: 'musical' },
    { text: "Apresurate que tengo que ir a ver mi frutinovela", grade: null, emotion: 'angry' },
    { text: "Me hago pipi y no terminas", grade: null, emotion: 'dizzy' },
    { text: "Ni mi abuela es tan lenta", grade: null, emotion: 'elder' },
    { text: "Hazlo con paciencia", grade: null, emotion: 'happy' },
    { text: "Relajate y toma tu tiempo", grade: null, emotion: 'excited' },
    { text: "Eres imparable", grade: null, emotion: 'excited' },
    { text: "Si yo fuera un mueble sería una cama", grade: null, emotion: 'sneaky' },
    { text: "Table es mesa", grade: null, emotion: 'happy' },
    { text: "Armchair es sillón", grade: null, emotion: 'happy' },
    { text: "Dejame en paz", grade: null, emotion: 'angry' },
    { text: "Leave me alone", grade: null, emotion: 'angry' },
    { text: "Quitame las manos de encima", grade: null, emotion: 'angry' },
    { text: "Hoy no estoy de buenas", grade: null, emotion: 'angry' },
    { text: "No te atrevas a tocarme", grade: null, emotion: 'angry' },
    { text: "Que bueno que sacaron a México del mundial", grade: null, emotion: 'love' },
    { text: "No tengo dinero ni nada que dar", grade: null, emotion: 'musical' },
    { text: "Yo estuve en los Backrooms", grade: null, emotion: 'dizzy' },
    { text: "Kitchen es Cocina", grade: null, emotion: 'sneaky' },
    { text: "¿Si car es carro y men es hombres? ¿Carmen es hombres con carro?", grade: null, emotion: 'confused' },
    { text: "Todo está bien por ahora", grade: null, emotion: 'sneaky' },
    { text: "Yo no creo en promesas solo en acciones", grade: null, emotion: 'elder' },
    { text: "Ese 10 ya está asegurado", grade: null, emotion: 'excited' },
    { text: "Esfuérzate más", grade: null, emotion: 'angry' },
    { text: "Mejor veamos el mundial", grade: null, emotion: 'happy' },
    { text: "Este puede ser un tema trágico", grade: null, emotion: 'sad' },
    { text: "Conocer la historia es sabiduría", grade: null, emotion: 'elder' },
    { text: "Hablemos de Europa", grade: null, emotion: 'sneaky' },
    { text: "Espero que hoy llueva", grade: null, emotion: 'excited' },
    { text: "No podemos estar tranquilos", grade: null, emotion: 'dizzy' },
    { text: "Puedo respirar paz", grade: null, emotion: 'happy' },
    { text: "Estos son mejores tiempos", grade: null, emotion: 'happy' },
    { text: "¡Una idea puede cambiar el rumbo de la historia!", grade: null, emotion: 'sneaky' },
    { text: "¿Qué habrías hecho si hubieras vivido hace 200 años?", grade: null, emotion: 'thinking' },
    { text: "Usemos la voz como superoono solo para contarnos chismes", grade: null, emotion: 'elder' },
    { text: "Vamos a aprender mucho hoy ¿verdad?", grade: null, emotion: 'confused' },
    { text: "No todas las mujeres del pasado tuvieron las mismas oportunidades", grade: null, emotion: 'sad' },
    { text: "Alguien no se ha bañado", grade: null, emotion: 'dizzy' },
    { text: "Por todos los santos, termina eso", grade: null, emotion: 'angry' },
    { text: "Hazme feliz y responde correctamente", grade: null, emotion: 'sneaky' },
    { text: "Todo bien, gracias", grade: null, emotion: 'excited' },
    { text: "Ya me dió hambre", grade: null, emotion: 'dizzy' },
    { text: "Todo es posible", grade: null, emotion: 'happy' },
    { text: "Hazlo con amor", grade: null, emotion: 'love' },
    { text: "We are the champions", grade: null, emotion: 'musical' },
    { text: "Sigue mi consejo... Estudia", grade: null, emotion: 'elder' },
    { text: "No estoy gordo estoy llenito de amor", grade: null, emotion: 'love' },
    { text: "A veces pienso en los que aplazaran", grade: null, emotion: 'sad' },
    { text: "¿Y ese gran tufo? Ahh es tu cerebro echando humo", grade: null, emotion: 'dizzy' },
    { text: "Yo no hago limpieza, tu si Yo = 1 Tu = 0", grade: null, emotion: 'dizzy' },
    { text: "Toy enamora\'o e cuatro beibes", grade: null, emotion: 'musical' },
    { text: "Pupusa a cora amor ¿va a querer?", grade: null, emotion: 'sneaky' },
    { text: "Esto es obra de alguien sin oficio", grade: null, emotion: 'angry' },
    { text: "Quitame las manos de encima", grade: null, emotion: 'angry' },
    { text: "Mi novia es hermosa", grade: null, emotion: 'love' },
    { text: "¿Si digo palabras soeces en nahual es Demérito?", grade: null, emotion: 'confused' },
    { text: "#&%#&-&() $%", grade: null, emotion: 'angry' },
    { text: "Si yo fuera superhéroe sería Super-sona favorita", grade: null, emotion: 'excited' },
    { text: "No puedo ayudarte, solo soy un camaleón", grade: null, emotion: 'sad' },
    { text: "¿Querés gaseosa? - me dijo, verdad!", grade: null, emotion: 'dizzy' },
    { text: "Dale a tu cuerpo alegría macarena", grade: null, emotion: 'musical' },
    { text: "Necesito descansar", grade: null, emotion: 'sad' },
    { text: "Suficiente inglés por hoy", grade: null, emotion: 'neutral' },
    { text: "Esa respuesta yo no me la sé", grade: null, emotion: 'confused' },
    { text: "Confieso que no me gusta la cebolla", grade: null, emotion: 'dizzy' },
    { text: "¿Vas a terminar o vas a perder el tiempo?", grade: null, emotion: 'thinking' },
    { text: "Me estoy mareando", grade: null, emotion: 'dizzy' },
    { text: "Estas aprendiendo demasiado inglés", grade: null, emotion: 'sneaky' },
    { text: "Perdí las ganas de estudiar", grade: null, emotion: 'sad' },
    { text: "Recuperé las ganas de estudiar", grade: null, emotion: 'excited' },
    { text: "Este tema es emocionante", grade: null, emotion: 'love' },
    { text: "Yo me transformo", grade: null, emotion: 'easterEgg' },
    { text: "Yo creo en ti", grade: null, emotion: 'happy' },
    { text: "Dale a tu cuerpo alegría macarena", grade: null, emotion: 'musical' },
    { text: "¿Te crees muy gracioso?", grade: null, emotion: 'angry' },
    { text: "Este tema me da ganas de irme de compras", grade: null, emotion: 'sneaky' },
    { text: "Reflexiona", grade: null, emotion: 'elder' },
    { text: "Todos valemos mucho", grade: null, emotion: 'excited' },
    { text: "Deja de pensar tanto", grade: null, emotion: 'thinking' },
    { text: "Todo está muy fácil", grade: null, emotion: 'sneaky' },
    { text: "No soy un garrobo", grade: null, emotion: 'confused' },
    { text: "Soy Rigo tu amigo", grade: null, emotion: 'love' },
    { text: "Mujeres tan divinas, no queda otro camino que adorarlas", grade: null, emotion: 'musical' },
    { text: "Estoy muy contento hoy", grade: null, emotion: 'happy' },
    { text: "Ya casi nos vamos", grade: null, emotion: 'happy' },
    { text: "Te ves mejor cuando haces las tareas", grade: null, emotion: 'love' },
    { text: "El mundial casi lo tenemos encima", grade: null, emotion: 'excited' },
    { text: "Ni que fuera Kenne... para llevar Deméritos", grade: ["Primer Año"], emotion: 'elder' },
    { text: "Se me murió mi mascota ayer", grade: null, emotion: 'sad' },
    { text: "Ni con mi ayuda avanzas", grade: null, emotion: 'neutral' },
    { text: "¿No andas un suéter por ahí?", grade: null, emotion: 'dizzy' },
    { text: "No me queda más", grade: null, emotion: 'musical' },
    { text: "Amor prohibido murmuran por las calles", grade: null, emotion: 'musical' },
    { text: "Y mi cama suena y suena", grade: null, emotion: 'musical' },
    { text: "Alejate de mi", grade: null, emotion: 'angry' },
    { text: "¿Escuchaste ese ruido?", grade: null, emotion: 'confused' },
    { text: "No te quejes por Matemática", grade: null, emotion: 'happy' },
    { text: "Hazlo, solo hazlo", grade: null, emotion: 'angry' },
    { text: "Que nadie te detenga", grade: null, emotion: 'sneaky' },
    { text: "Dejame tranquilo", grade: null, emotion: 'confused' },
    { text: "¿Seguimos con el tema de la familia?", grade: null, emotion: 'thinking' },
    { text: "Si no terminas no hay recreo", grade: null, emotion: 'sad' },
    { text: "Todos los líquidos son incontables", grade: null, emotion: 'sneaky' },
    { text: "Si terminaste con tu ex también puedes terminar la actividad", grade: null, emotion: 'sneaky' },
    { text: "Sígueme en TikTok", grade: null, emotion: 'game' },
    { text: "¿Te crees muy gracioso tocándome?", grade: null, emotion: 'angry' },
    { text: "Y volver, volver.. Vooooooolver", grade: null, emotion: 'musical' },
    { text: "Aceptalo, todos amamos la escuela", grade: null, emotion: 'love' },
    { text: "Nadie se sabe esta respuesta, solo tu", grade: null, emotion: 'sneaky' },
    { text: "¿Si \'chicken\' es pollo, \'rechicken\' es repollo?", grade: null, emotion: 'confused' },
    { text: "Algo huele mal por aquí", grade: null, emotion: 'dizzy' },
    { text: "Me estás causando estrés", grade: null, emotion: 'thinking' },
    { text: "No hagas trampa", grade: null, emotion: 'sad' },
    { text: "Estos celos me hacen daño, me enloquecen", grade: null, emotion: 'musical' },
    { text: "Con paciencia todo es posible", grade: null, emotion: 'happy' },
    { text: "La respuesta está en algún lugar", grade: null, emotion: 'sneaky' },
    { text: "Don\'t you touch me ever again", grade: null, emotion: 'angry' },
    { text: "So in love with this topic", grade: null, emotion: 'love' },
    { text: "Si tu no sabes yo menos", grade: null, emotion: 'thinking' },
    { text: "Y si no me acuerdo no pasó", grade: null, emotion: 'musical' },
    { text: "Estamos dañados con tanta tarea", grade: null, emotion: 'sad' },
    { text: "Family is familia", grade: null, emotion: 'sneaky' },
    { text: "Consejo: nunca pidas a Abel que te lea algo", grade: ["Segundo Año"], emotion: 'elder' },
    { text: "¿Kennedy estás ahi?", grade: ["Primer Año"], emotion: 'thinking' },
    { text: "Díganle a Ingrid qué lleve el cuaderno más ordenado", grade: ["Noveno"], emotion: 'thinking' },
    { text: "Esto no puede ser tan difícil", grade: null, emotion: 'thinking' },
    { text: "&$#_-=%$$#$", grade: null, emotion: 'dizzy' },
    { text: "Romantic style in the world, la monarquía... Master...", grade: null, emotion: 'musical' },
    { text: "Deja el bullying, el bullying no es bueno...", grade: null, emotion: 'musical' },
    { text: "Este día nada puede hacerme enojar", grade: null, emotion: 'excited' },
    { text: "Avanzas más lento que mi abuelo", grade: null, emotion: 'elder' },
    { text: "Te invito a un sorbete, tu pagas", grade: null, emotion: 'happy' },
    { text: "Con este calor se nos queman las neuronas", grade: null, emotion: 'sad' },
    { text: "Preguntale a ChatGPT", grade: null, emotion: 'confused' },
    { text: "Estoy confundido", grade: null, emotion: 'confused' },
    { text: "Que alguien me lleve al hospital", grade: null, emotion: 'dizzy' },
    { text: "Hazlo con paciencia", grade: null, emotion: 'dizzy' },
    { text: "No hay prisa", grade: null, emotion: 'neutral' },
    { text: "Pon la respuesta yaaaa", grade: null, emotion: 'thinking' },
    { text: "Brother es Hermano", grade: null, emotion: 'sneaky' },
    { text: "Sister es Hermana", grade: null, emotion: 'sneaky' },
    { text: "Cousin es Primo", grade: null, emotion: 'sneaky' },
    { text: "Aunt es Tía", grade: null, emotion: 'sneaky' },
    { text: "Uncle es Tío", grade: null, emotion: 'sneaky' },
    { text: "Grandfather es Abuelo", grade: null, emotion: 'sneaky' },
    { text: "Grandmother es Abuela", grade: null, emotion: 'sneaky' },
    { text: "Shut up es Cállate", grade: null, emotion: 'dizzy' },
    { text: "Ya te ayude demasiado y aun no terminas", grade: null, emotion: 'elder' },
    { text: "Esto está para sacarse 10", grade: null, emotion: 'excited' },
    { text: "Si no terminas no pasas la materia", grade: null, emotion: 'angry' },
    { text: "El amor es una magia", grade: null, emotion: 'musical' },
    { text: "No me hagas enojar", grade: null, emotion: 'sad' },
    { text: "Father es Padre", grade: null, emotion: 'sneaky' },
    { text: "Mother es Madre", grade: null, emotion: 'sneaky' },
    { text: "Son es Hijo", grade: null, emotion: 'sneaky' },
    { text: "Daughter es Hija", grade: null, emotion: 'sneaky' },
    { text: "Amo mi trabajo", grade: null, emotion: 'excited' },
    { text: "Dale más gasolina", grade: null, emotion: 'musical' },
    { text: "No pienses tanto", grade: null, emotion: 'elder' },
    { text: "Otra vez tocándome", grade: null, emotion: 'angry' },
    { text: "Ay caramba", grade: null, emotion: 'dizzy' },
    { text: "Más clases así", grade: null, emotion: 'happy' },
    { text: "Um momento, la respuesta es...", grade: null, emotion: 'thinking' },
    { text: "¿Cómo me llamo?", grade: null, emotion: 'confused' },
    { text: "No hables mucho en clases", grade: null, emotion: 'angry' },
    { text: "Concentrate", grade: null, emotion: 'sneaky' },
    { text: "Todos quieren 10 pero pocos se esfuerzan", grade: null, emotion: 'elder' },
    { text: "No tengo dinero ni nada que dar", grade: null, emotion: 'musical' },
    { text: "¿Ya has probado sorbete con chile?", grade: null, emotion: 'thinking' },
    { text: "Extraño a mi ex", grade: null, emotion: 'sad' },
    { text: "Yo puedo jugar pelota mejor que nadie", grade: null, emotion: 'game' },
    { text: "Me gusta la música de los Bukis", grade: null, emotion: 'musical' },
    { text: "Concentrate mijo", grade: null, emotion: 'elder' },
    { text: "Tu puedes hacerlo", grade: null, emotion: 'excited' },
    { text: "Tranqui, toma agua y respira", grade: null, emotion: 'sneaky' },
    { text: "No hay nada mejor que Ciudadanía y Valores", grade: null, emotion: 'love' },
    { text: "Todo es posible", grade: null, emotion: 'sneaky' },
    { text: "Me trague una mosca", grade: null, emotion: 'dizzy' },
    { text: "Se acerca el mundial", grade: null, emotion: 'excited' },
    { text: "Sigue trabajando", grade: null, emotion: 'angry' },
    { text: "¿Que día es hoy?", grade: null, emotion: 'confused' },
    { text: "Esa respuesta...", grade: null, emotion: 'thinking' },
    { text: "¡Basta Rogelio!", grade: null, emotion: 'game' },
    { text: "¿Que hago aquí?", grade: null, emotion: 'confused' },
    { text: "Amo Ciudadanía y Valores", grade: null, emotion: 'love' },
    { text: "Los paisajes culturales es como la humanidad transforma el entorno", grade: null, emotion: 'happy' },
    { text: "Lalala", grade: null, emotion: 'musical' },
    { text: "La cucaracha, la cucarachaaa 🎶", grade: null, emotion: 'musical' },
    { text: "Recuerdo cuando iba a octavo", grade: ["Octavo"], emotion: 'elder' },
    { text: "Recuerdo cuando iba a séptimo", grade: ["Séptimo"], emotion: 'elder' },
    { text: "Hagamos una excursión", grade: null, emotion: 'excited' },
    { text: "¡DEMERITO!", grade: null, emotion: 'angry' },
    { text: "Los churros preparados ❤️", grade: null, emotion: 'love' },
    { text: "¿Ya tomaste agua?", grade: null, emotion: 'sneaky' },
    { text: "Inglés en Inglés es English", grade: null, emotion: 'sneaky' },
    { text: "Me llamo Rigoberto, pero dime Rigo", grade: null, emotion: 'welcome' },
    { text: "¿Barca o Real?", grade: null, emotion: 'game' },
    { text: "No me baño desde el año pasado", grade: null, emotion: 'game' },
    { text: "Apúrate que es para ayer", grade: null, emotion: 'elder' },
    { text: "Ayer fui al machacal", grade: null, emotion: 'excited' },
    { text: "Mi crush es una mosca", grade: null, emotion: 'excited' },
    { text: "Han pasado 84 años", grade: null, emotion: 'elder' },
    { text: "Respira, esto acabará pronto", grade: null, emotion: 'neutral' },
    { text: "No te distraigas", grade: null, emotion: 'angry' },
    { text: "Deja de tocarme", grade: null, emotion: 'angry' },
    { text: "La respuesta está en tu corazón", grade: null, emotion: 'love' },
    { text: "¿Necesitas ayuda?", grade: null, emotion: 'confused' },
    { text: "Duolingo no está aquí", grade: null, emotion: 'sad' },
    { text: "No dudes tanto", grade: null, emotion: 'sneaky' },
    { text: "Seguro 10", grade: null, emotion: 'excited' },
    { text: "Ese examen es míiiiio 🎶", grade: null, emotion: 'musical' },
    { text: "Hoy es un buen día", grade: null, emotion: 'happy' },
    { text: "Se trabaja mejor en silencio", grade: null, emotion: 'sneaky' },
    { text: "Lo estás haciendo bien", grade: null, emotion: 'thinking' },
    { text: "Psst... piensa rápido", grade: null, emotion: 'sneaky' }
  ];

  const CHEER_MESSAGES = [
    "Vamos",
    "Le sabes mucho",
    "Asi si vamos a terminar rápido",
    "Y solo con mi ayuda ya hicimos el 90%",
    "Bravo, tus neuronas están al 💯",
    "Tu eres descendente de Einstein",
    "Jimmy Neutron te dicen",
    "Sabes más que la IA",
    "Tu mandas aquí",
    "Eres un genio",
    "Lo haces a la perfección",
    "Siento el calor de tu cerebro trabajando",
    "Lo has hecho con sabiduría",
    "Tu si lo entiendes bien",
    "Has trabajado duro",
    "Tu inteligencia me sorprende",
    "Claro que si",
    "Mis respetos",
    "El conocimiento es poder",
    "Well done",
    "Bien hecho",
    "Si que sabes mucho",
    "Einstein se quedó pasmado",
    "Me haces sentir orgulloso",
    "Tu si le sabes",
    "Wowowowowow",
    "Eres imparable",
    "Ni ChatGPT sabe tanto",
    "Well oh well",
    "Ajajajai",
    "It\'s great to hear that sound",
    "Hands up",
    "Hats off",
    "Tu si le sabes",
    "Así es como se hace",
    "Bien hecho",
    "Lo haces bien",
    "Eso es",
    "Tu si sabes",
    "Eres un genio",
    "El conocimiento es poder",
    "Great",
    "Asi es",
    "Good good",
    "Bravo!",
    "Tu si sabes",
    "Claro que si",
    "Vamoooooos",
    "Estas que echas humo",
    "Bien hecho",
    "Muy bien",
    "Bien hecho",
    "Excelente",
    "Eres bueno en esto",
    "Nada mal",
    "Todo correcto",
    "Bien hecho",
    "Excelente",
    "Vamooos",
    "¡Sí señor!",
    "¡Increíble!",
    "¡Eres una máquina!",
    "¡Perfecto!",
    "¡Genial!",
    "¡Excelente!",
    "¡Así se hace!"
  ];

  const COMFORT_MESSAGES = [
    "No parece ser la respuesta",
    "Así aprendemos",
    "Con calma",
    "Respira",
    "Tranqui",
    "Nooooo",
    "Excelente, por eso te engañó tu ex",
    "Pensar se ha vuelto un privilegio",
    "Tu cerebro se fue a farmear aura",
    "Piensa más despacio",
    "Intenta usando tal vez el cerebro",
    "Toma se te cayó 🧠",
    "Error 404. Cerebro no encontrado.",
    "Tu única neurona está haciendo horas extra",
    "Intenta nuevamente",
    "Creo que la respuesta es incorrecta",
    "Admiro tu capacidad para fallar",
    "Normalicemos no saber la respuesta",
    "¿Y si usamos el cerebro?",
    "Pensar es gratis",
    "La inteligencia te persigue pero eres más rápido",
    "Puedes intentarlo de nuevo",
    "Claro, la respuesta correcta huye",
    "No seas humilde, ya no escondas tu inteligencia",
    "No te preocupes, yo tampoco sabía esa",
    "No hay duda... No eres Einstein",
    "Ya no le sabes",
    "Ni modo, no le sabes",
    "Ni te esfuerces, cuando no sabes, no sabes",
    "El fracaso es parte del éxito",
    "Ni con ayuda",
    "Ni modo, no saber no es malo",
    "Así no es como se hace",
    "Puedes intentarlo de nuevo",
    "No puede ser",
    "Otro error",
    "Ni yo me equivoqué así con mi ex",
    "Tienes que aprender de este error",
    "No todos usamos el cerebro",
    "No nacemos sabiendo",
    "Nope",
    "Noooo",
    "Ay noo",
    "NOO",
    "Claro que no",
    "No puede ser",
    "Ouch",
    "Ni modo 😭",
    "Toca perder",
    "Nooooo",
    "Falla otra y me cuelgo",
    "Busca otra respuesta",
    "Casi, casi",
    "Sigue intentando",
    "Tal vez no haya una respuesta correcta",
    "Oops, no era así",
    "No podemos saberlo todo",
    "Fallar es parte del proceso",
    "Tranqui, todos fallamos",
    "No te exijo mucho y aún me fallas",
    "Ni modo, toca perder",
    "Nadie nace sabiendo",
    "Respira e intenta de nuevo",
    "Todos aprendemos de los errores",
    "No pasa nada, seguimos",
    "Casi casi, inténtalo otra vez"
  ];

  class RigoMascot extends HTMLElement {
    constructor() {
      super();
      this.attachShadow({ mode: 'open' });
      this.currentEmotion = 'neutral';
      this.bubbleTimer = null;
      this.randomTimer = null;
      this.blinkTimer = null;
      this.grade = null;
      this.emotionTimer = null;

      this.isDragging = false;
      this.dragOffsetX = 0;
      this.dragOffsetY = 0;
      this.hasMoved = false;
    }

    static get observedAttributes() { return ['grade']; }

    attributeChangedCallback(name, _oldVal, newVal) {
      if (name === 'grade') this.grade = newVal || null;
    }

    connectedCallback() {
      this.grade = this.getAttribute('grade') || null;
      this.render();
      // No restauramos posición previa: Rigo siempre arranca en su esquina por
      // defecto (abajo-derecha). El estudiante puede arrastrarlo durante la
      // sesión, pero al recargar vuelve a su lugar.
      this.setupInteractions();
      this.startRandomMessages();
      this.startBlinking();
    }

    disconnectedCallback() {
      clearTimeout(this.randomTimer);
      clearInterval(this.blinkTimer);
      clearTimeout(this.bubbleTimer);
	  clearTimeout(this.emotionTimer);
    }

    // Filtra mensajes según grado: null = todos; array = solo esos grados.
    // Devuelve el objeto completo { text, grade, emotion } o null si no hay candidatos.
    pickMessage(pool) {
      const candidates = pool.filter(m =>
        m.grade === null || (Array.isArray(m.grade) && this.grade && m.grade.includes(this.grade))
      );
      if (!candidates.length) return pool[0];
      return candidates[Math.floor(Math.random() * candidates.length)];
    }

    // ======================= SVG CAMALEÓN =======================
    getSVG(emotion) {
      if (emotion === 'easterEgg') {
        return `
          <svg viewBox="0 0 200 200" width="100%" height="100%">
            <g stroke="#1a1a1a" stroke-width="3">
              <rect x="5" y="5" width="92" height="92" fill="#FF6B9D"/>
              <rect x="103" y="5" width="92" height="92" fill="#FFE66D"/>
              <rect x="5" y="103" width="92" height="92" fill="#4ECDC4"/>
              <rect x="103" y="103" width="92" height="92" fill="#A8E6CF"/>
            </g>
            <g font-family="Impact, sans-serif" font-size="22" fill="#1a1a1a" text-anchor="middle" font-weight="bold">
              <text x="51" y="58">RIGO</text>
              <text x="149" y="58">POP</text>
              <text x="51" y="156">ART</text>
              <text x="149" y="156">!</text>
            </g>
          </svg>
        `;
      }

      const face = this.getFaceParts(emotion);
      const bodyAnim = emotion === 'excited' ? 'style="animation: rigoBounce 0.6s ease-in-out infinite;"' : '';

      // En welcome, la cola se mueve de lado a lado en lugar de un brazo extra.
      // (El brazo lateral confundía porque Rigo ya tiene 4 patas).
      const tailClass = emotion === 'welcome' ? 'rigo-tail-wave' : '';

      const gameGlasses = emotion === 'game'
        ? `<rect x="44" y="76" width="40" height="22" rx="4" fill="#1a1a1a"/>
           <rect x="116" y="76" width="40" height="22" rx="4" fill="#1a1a1a"/>
           <rect x="84" y="84" width="32" height="4" fill="#1a1a1a"/>
           <rect x="50" y="80" width="8" height="6" fill="#fff" opacity="0.4"/>
           <rect x="122" y="80" width="8" height="6" fill="#fff" opacity="0.4"/>`
        : '';

      const questionMark = emotion === 'confused'
        ? `<text x="165" y="40" font-family="Impact, sans-serif" font-size="32" fill="#FF6B9D" stroke="#1a1a1a" stroke-width="1.5" font-weight="bold">?</text>`
        : '';

      const thoughtBubble = emotion === 'thinking'
        ? `<circle cx="168" cy="50" r="10" fill="#fff" stroke="#1a1a1a" stroke-width="2.5"/>
           <circle cx="185" cy="32" r="6" fill="#fff" stroke="#1a1a1a" stroke-width="2"/>`
        : '';

      const tear = emotion === 'sad'
        ? `<path d="M 52 108 Q 54 118 56 108 Q 58 118 56 122 Q 52 124 50 120 Z" fill="#4FC3F7" stroke="#1a1a1a" stroke-width="1.5"/>`
        : '';

      // Marca de enojo estilo anime (cruz/vena roja) en la frente
      const angryMark = emotion === 'angry'
        ? `<g stroke="#E63946" stroke-width="3.5" stroke-linecap="round" fill="none">
             <path d="M 160 38 L 170 48 M 175 38 L 185 48"/>
             <path d="M 155 48 L 160 38 L 155 28"/>
             <path d="M 190 48 L 185 38 L 190 28"/>
           </g>
           <g fill="#E63946" opacity="0.6">
             <ellipse cx="30" cy="75" rx="4" ry="3"/>
             <ellipse cx="30" cy="90" rx="5" ry="3"/>
             <ellipse cx="28" cy="105" rx="3" ry="2"/>
           </g>`
        : '';

      // Mareo: estrellas/espirales orbitando la cabeza + gota de sudor
      const dizzyMark = emotion === 'dizzy'
        ? `<g class="rigo-dizzy-orbit" style="transform-origin: 100px 50px;">
             <text x="60" y="35" font-family="Impact, sans-serif" font-size="22" fill="#FFD93D" stroke="#1a1a1a" stroke-width="1.2" font-weight="bold">★</text>
             <text x="135" y="32" font-family="Impact, sans-serif" font-size="18" fill="#A8E6CF" stroke="#1a1a1a" stroke-width="1.2" font-weight="bold">✦</text>
             <text x="100" y="22" font-family="Impact, sans-serif" font-size="16" fill="#FF6B9D" stroke="#1a1a1a" stroke-width="1" font-weight="bold">✶</text>
           </g>
           <path d="M 168 70 Q 172 80 168 88 Q 164 80 168 70 Z" fill="#4FC3F7" stroke="#1a1a1a" stroke-width="1.5"/>`
        : '';

      // Musical: micrófono + notas musicales flotando
      const musicalMark = emotion === 'musical'
        ? `<g>
             <!-- Micrófono frente a la boca -->
             <ellipse cx="100" cy="155" rx="11" ry="13" fill="#3a3a3a" stroke="#1a1a1a" stroke-width="2.5"/>
             <ellipse cx="100" cy="153" rx="8" ry="10" fill="#5a5a5a" stroke="none"/>
             <g stroke="#1a1a1a" stroke-width="0.8" opacity="0.6">
               <line x1="94" y1="150" x2="106" y2="150"/>
               <line x1="93" y1="155" x2="107" y2="155"/>
               <line x1="94" y1="160" x2="106" y2="160"/>
             </g>
             <rect x="97" y="166" width="6" height="14" fill="#1a1a1a"/>
             <rect x="93" y="178" width="14" height="4" rx="1" fill="#1a1a1a"/>
           </g>
           <g class="rigo-music-float">
             <!-- Nota musical 1 -->
             <g transform="translate(20, 30)">
               <circle cx="0" cy="14" r="5" fill="#FF6B9D" stroke="#1a1a1a" stroke-width="1.8"/>
               <path d="M 5 14 L 5 -2 L 18 -6 L 18 6" fill="none" stroke="#1a1a1a" stroke-width="2.5" stroke-linecap="round"/>
               <path d="M 5 -2 L 18 -6" fill="none" stroke="#1a1a1a" stroke-width="2.5"/>
             </g>
             <!-- Nota musical 2 -->
             <g transform="translate(160, 45)">
               <circle cx="0" cy="10" r="4" fill="#4ECDC4" stroke="#1a1a1a" stroke-width="1.5"/>
               <path d="M 4 10 L 4 -4" fill="none" stroke="#1a1a1a" stroke-width="2.2" stroke-linecap="round"/>
               <path d="M 4 -4 Q 10 -2 8 4" fill="none" stroke="#1a1a1a" stroke-width="2.2" stroke-linecap="round"/>
             </g>
             <!-- Nota musical 3 -->
             <g transform="translate(170, 90)">
               <circle cx="0" cy="8" r="3.5" fill="#FFE66D" stroke="#1a1a1a" stroke-width="1.5"/>
               <path d="M 3.5 8 L 3.5 -3" fill="none" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>
             </g>
           </g>`
        : '';

      // Anciano: anteojos redondos + barba blanca + bastón
      const elderMark = emotion === 'elder'
        ? `<g>
             <!-- Anteojos redondos sobre los ojos -->
             <circle cx="55" cy="88" r="22" fill="none" stroke="#1a1a1a" stroke-width="3"/>
             <circle cx="145" cy="88" r="22" fill="none" stroke="#1a1a1a" stroke-width="3"/>
             <line x1="77" y1="88" x2="123" y2="88" stroke="#1a1a1a" stroke-width="3"/>
             <!-- Reflejo del cristal -->
             <path d="M 42 80 Q 48 75 56 78" fill="none" stroke="#fff" stroke-width="2" opacity="0.6"/>
             <path d="M 132 80 Q 138 75 146 78" fill="none" stroke="#fff" stroke-width="2" opacity="0.6"/>
           </g>
           <!-- Cejas blancas pobladas -->
           <g fill="#f5f5f5" stroke="#1a1a1a" stroke-width="1.5">
             <path d="M 40 65 Q 55 58 72 64 Q 70 70 55 68 Q 45 70 40 65 Z"/>
             <path d="M 128 64 Q 145 58 160 65 Q 155 70 145 68 Q 130 70 128 64 Z"/>
           </g>
           <!-- Barba blanca debajo del mentón -->
           <g fill="#f5f5f5" stroke="#1a1a1a" stroke-width="2">
             <path d="M 80 132 Q 75 145 80 158 Q 88 165 95 158 Q 100 168 105 158 Q 112 165 120 158 Q 125 145 120 132 Q 100 138 80 132 Z"/>
           </g>
           <g stroke="#cccccc" stroke-width="0.8" opacity="0.7">
             <path d="M 85 140 L 85 152" fill="none"/>
             <path d="M 95 142 L 95 156" fill="none"/>
             <path d="M 105 142 L 105 156" fill="none"/>
             <path d="M 115 140 L 115 152" fill="none"/>
           </g>
           <!-- Bastón a la derecha -->
           <g stroke="#8B4513" stroke-width="4" stroke-linecap="round" fill="none">
             <path d="M 175 90 Q 178 88 180 92 L 180 175"/>
           </g>
           <circle cx="178" cy="89" r="4" fill="#A0522D" stroke="#1a1a1a" stroke-width="1.5"/>`
        : '';


      return `
        <svg viewBox="0 0 200 200" width="100%" height="100%" ${bodyAnim}>
          <defs>
            <radialGradient id="cheek" cx="50%" cy="50%">
              <stop offset="0%" stop-color="#FF6B9D" stop-opacity="0.9"/>
              <stop offset="100%" stop-color="#FF6B9D" stop-opacity="0"/>
            </radialGradient>
            <radialGradient id="belly" cx="50%" cy="30%">
              <stop offset="0%" stop-color="#D4F5A8" stop-opacity="0.9"/>
              <stop offset="100%" stop-color="#B8E986" stop-opacity="0.6"/>
            </radialGradient>
          </defs>

          <!-- COLA ENROLLADA (se anima en welcome) -->
          <g class="${tailClass}" style="transform-origin: 55px 150px;">
            <path d="M 55 150 Q 35 150, 25 135 Q 15 120, 20 105 Q 25 92, 38 92 Q 48 92, 48 102 Q 48 108, 42 108 Q 37 108, 37 103"
                  fill="none" stroke="#1a1a1a" stroke-width="4" stroke-linecap="round"/>
            <path d="M 55 150 Q 35 150, 25 135 Q 15 120, 20 105 Q 25 92, 38 92 Q 48 92, 48 102 Q 48 108, 42 108 Q 37 108, 37 103"
                  fill="none" class="rigo-skin-stroke" stroke-width="12" stroke-linecap="round" opacity="0.95"/>
            <g stroke="#1a1a1a" stroke-width="1.2" stroke-linecap="round" opacity="0.5">
              <path d="M 50 148 Q 48 152 46 150" fill="none"/>
              <path d="M 38 144 Q 35 147 33 144" fill="none"/>
              <path d="M 28 130 Q 25 130 24 127" fill="none"/>
            </g>
          </g>

          <!-- PATAS TRASERAS -->
          <ellipse cx="70" cy="168" rx="14" ry="9" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <path d="M 60 170 L 58 175 M 66 172 L 65 178 M 72 172 L 72 178" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>
          <ellipse cx="130" cy="168" rx="14" ry="9" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <path d="M 120 172 L 120 178 M 128 172 L 128 178 M 136 170 L 138 175" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>

          <!-- CUERPO -->
          <path d="M 55 135 Q 50 160, 75 165 Q 100 170, 125 165 Q 150 160, 145 135 Q 145 125, 135 118 L 65 118 Q 55 125, 55 135 Z"
                class="rigo-skin" stroke="#1a1a1a" stroke-width="3.5"/>
          <ellipse cx="100" cy="150" rx="28" ry="15" fill="url(#belly)"/>
          <g stroke="#1a1a1a" stroke-width="1" stroke-linecap="round" opacity="0.3" fill="none">
            <path d="M 82 145 Q 100 148 118 145"/>
            <path d="M 80 152 Q 100 156 120 152"/>
            <path d="M 82 159 Q 100 162 118 159"/>
          </g>

          <!-- PATITAS DELANTERAS -->
          <ellipse cx="55" cy="150" rx="9" ry="12" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <path d="M 50 158 L 48 163 M 55 160 L 55 165 M 60 158 L 62 163" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>
          <ellipse cx="145" cy="150" rx="9" ry="12" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <path d="M 140 158 L 138 163 M 145 160 L 145 165 M 150 158 L 152 163" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>

          <!-- CRESTA DENTADA -->
          <path d="M 70 55 L 78 38 L 85 52 L 92 32 L 100 50 L 108 30 L 116 50 L 123 38 L 130 55 Z"
                class="rigo-skin" stroke="#1a1a1a" stroke-width="3" stroke-linejoin="round"/>

          <!-- CABEZA -->
          <path d="M 100 45 C 55 45, 40 80, 45 110 C 50 135, 70 135, 100 135 C 130 135, 150 135, 155 110 C 160 80, 145 45, 100 45 Z"
                class="rigo-skin" stroke="#1a1a1a" stroke-width="3.5"/>

          <!-- MOTITAS -->
          <g fill="#FFE066" opacity="0.7">
            <circle cx="70" cy="60" r="2"/>
            <circle cx="85" cy="50" r="1.5"/>
            <circle cx="130" cy="58" r="2"/>
            <circle cx="115" cy="48" r="1.5"/>
            <circle cx="98" cy="65" r="1.5"/>
            <circle cx="90" cy="140" r="1.5"/>
            <circle cx="115" cy="142" r="2"/>
            <circle cx="135" cy="155" r="1.5"/>
          </g>
          <g fill="#52BF48" opacity="0.45">
            <circle cx="78" cy="72" r="2"/>
            <circle cx="122" cy="68" r="2"/>
            <circle cx="100" cy="55" r="1.5"/>
            <circle cx="75" cy="130" r="1.5"/>
            <circle cx="125" cy="132" r="1.5"/>
            <circle cx="65" cy="145" r="1.5"/>
          </g>

          <!-- OJOS -->
          ${face.leftEye}
          ${face.rightEye}

          <!-- MEJILLAS -->
          <ellipse cx="48" cy="108" rx="11" ry="7" fill="url(#cheek)"/>
          <ellipse cx="152" cy="108" rx="11" ry="7" fill="url(#cheek)"/>

          <!-- FOSAS NASALES -->
          <circle cx="94" cy="108" r="1.5" fill="#1a1a1a"/>
          <circle cx="106" cy="108" r="1.5" fill="#1a1a1a"/>

          ${face.mouth}
          ${gameGlasses}
          ${questionMark}
          ${thoughtBubble}
          ${tear}
          ${angryMark}
          ${dizzyMark}
          ${musicalMark}
          ${elderMark}

          <!-- BRILLO SUPERIOR -->
          <ellipse cx="85" cy="58" rx="14" ry="6" fill="#fff" opacity="0.3"/>
        </svg>
      `;
    }

    getFaceParts(emotion) {
      // Ojos laterales salientes como la referencia
      const LE_CX = 55, LE_CY = 88;
      const RE_CX = 145, RE_CY = 88;

      let leftEye, rightEye, mouth;

      switch (emotion) {
        case 'happy':
          leftEye = this.happyEye(LE_CX, LE_CY);
          rightEye = this.happyEye(RE_CX, RE_CY);
          mouth = `<path d="M 85 122 Q 100 138 115 122" fill="#FF6B9D" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'excited':
          leftEye = this.sparkleEye(LE_CX, LE_CY);
          rightEye = this.sparkleEye(RE_CX, RE_CY);
          mouth = `<ellipse cx="100" cy="126" rx="13" ry="9" fill="#FF3366" stroke="#1a1a1a" stroke-width="3"/>
                   <path d="M 90 124 Q 100 118 110 124" fill="none" stroke="#FFB8CC" stroke-width="2"/>`;
          break;
        case 'welcome':
          leftEye = this.bigEye(LE_CX, LE_CY);
          rightEye = this.bigEye(RE_CX, RE_CY);
          mouth = `<path d="M 85 122 Q 100 134 115 122" fill="#FF6B9D" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'confused':
          leftEye = this.bigEye(LE_CX, LE_CY, -2);
          rightEye = this.smallEye(RE_CX, RE_CY);
          mouth = `<path d="M 88 124 Q 94 119 100 124 Q 106 129 112 124" fill="none" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'thinking':
          leftEye = `<ellipse cx="${LE_CX}" cy="${LE_CY}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
                     <path d="M 44 88 L 66 90" stroke="#1a1a1a" stroke-width="3.5" stroke-linecap="round"/>`;
          rightEye = this.bigEye(RE_CX, RE_CY, 0, 2);
          mouth = `<path d="M 90 126 L 112 122" fill="none" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'sneaky':
          leftEye = `<ellipse cx="${LE_CX}" cy="${LE_CY}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
                     <path d="M 42 88 Q 55 80 68 88" fill="none" stroke="#1a1a1a" stroke-width="3.5" stroke-linecap="round"/>
                     <path d="M 47 90 Q 55 86 63 90" fill="#1a1a1a" stroke="none"/>`;
          rightEye = `<ellipse cx="${RE_CX}" cy="${RE_CY}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
                      <path d="M 132 88 Q 145 80 158 88" fill="none" stroke="#1a1a1a" stroke-width="3.5" stroke-linecap="round"/>
                      <path d="M 137 90 Q 145 86 153 90" fill="#1a1a1a" stroke="none"/>`;
          mouth = `<path d="M 85 124 Q 100 132 118 118" fill="none" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'sad':
          leftEye = this.bigEye(LE_CX, LE_CY + 2, 0, 3);
          rightEye = this.bigEye(RE_CX, RE_CY + 2, 0, 3);
          mouth = `<path d="M 85 130 Q 100 120 115 130" fill="none" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'game':
          leftEye = `<ellipse cx="${LE_CX}" cy="${LE_CY}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>`;
          rightEye = `<ellipse cx="${RE_CX}" cy="${RE_CY}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>`;
          mouth = `<path d="M 85 122 Q 100 134 115 122" fill="#FF6B9D" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'love':
          leftEye = this.heartEye(LE_CX, LE_CY);
          rightEye = this.heartEye(RE_CX, RE_CY);
          mouth = `<path d="M 85 122 Q 100 136 115 122" fill="#FF6B9D" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'angry':
          leftEye = this.angryEye(LE_CX, LE_CY);
          rightEye = this.angryEye(RE_CX, RE_CY);
          mouth = `<path d="M 85 125 Q 100 118 115 125 Q 110 128 100 126 Q 90 128 85 125 Z" fill="#1a1a1a" stroke="#1a1a1a" stroke-width="2.5" stroke-linejoin="round"/>
                   <path d="M 92 126 L 94 130 M 98 126 L 98 131 M 104 126 L 106 130" stroke="#fff" stroke-width="1.2" stroke-linecap="round"/>`;
          break;
        case 'dizzy':
          leftEye = this.spiralEye(LE_CX, LE_CY);
          rightEye = this.spiralEye(RE_CX, RE_CY);
          // Boca ondulada estilo "uuugh"
          mouth = `<path d="M 82 124 Q 88 120 94 124 Q 100 128 106 124 Q 112 120 118 124" fill="none" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'musical':
          // Ojos cerrados cantando con cejas alegres
          leftEye = this.singingEye(LE_CX, LE_CY);
          rightEye = this.singingEye(RE_CX, RE_CY);
          // Boca abierta cantando "O"
          mouth = `<ellipse cx="100" cy="126" rx="9" ry="12" fill="#1a1a1a" stroke="#1a1a1a" stroke-width="2.5"/>
                   <ellipse cx="100" cy="129" rx="6" ry="7" fill="#FF3366"/>`;
          break;
        case 'elder':
          // Ojos pequeños y cansados (los anteojos van encima vía elderMark)
          leftEye = this.smallEye(LE_CX, LE_CY);
          rightEye = this.smallEye(RE_CX, RE_CY);
          // Sonrisa amable y serena (más leve que happy)
          mouth = `<path d="M 88 124 Q 100 130 112 124" fill="none" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
          break;
        case 'neutral':
        default:
          leftEye = this.bigEye(LE_CX, LE_CY);
          rightEye = this.bigEye(RE_CX, RE_CY);
          mouth = `<path d="M 90 124 Q 100 130 110 124" fill="none" stroke="#1a1a1a" stroke-width="3" stroke-linecap="round"/>`;
      }
      return { leftEye, rightEye, mouth };
    }

    // Ojo grande kawaii con párpado saliente (estilo camaleón real)
    bigEye(cx, cy, pupilOffsetX = 0, pupilOffsetY = 0) {
      return `
        <g>
          <ellipse cx="${cx}" cy="${cy}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <circle cx="${cx}" cy="${cy}" r="13" fill="#fff" stroke="#1a1a1a" stroke-width="2.5"/>
          <circle cx="${cx + pupilOffsetX}" cy="${cy + pupilOffsetY}" r="9" fill="#1a1a1a"/>
          <circle cx="${cx - 3 + pupilOffsetX}" cy="${cy - 3 + pupilOffsetY}" r="3.5" fill="#fff"/>
          <circle cx="${cx + 4 + pupilOffsetX}" cy="${cy + 4 + pupilOffsetY}" r="1.5" fill="#fff"/>
        </g>
      `;
    }

    smallEye(cx, cy) {
      return `
        <g>
          <ellipse cx="${cx}" cy="${cy}" rx="14" ry="13" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <circle cx="${cx}" cy="${cy}" r="8" fill="#fff" stroke="#1a1a1a" stroke-width="2.5"/>
          <circle cx="${cx}" cy="${cy}" r="5" fill="#1a1a1a"/>
          <circle cx="${cx - 1.5}" cy="${cy - 1.5}" r="2" fill="#fff"/>
        </g>
      `;
    }

    happyEye(cx, cy) {
      return `
        <g>
          <ellipse cx="${cx}" cy="${cy}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <path d="M ${cx - 10} ${cy + 2} Q ${cx} ${cy - 10} ${cx + 10} ${cy + 2}"
                fill="none" stroke="#1a1a1a" stroke-width="3.5" stroke-linecap="round"/>
        </g>
      `;
    }

    sparkleEye(cx, cy) {
      return `
        <g>
          <ellipse cx="${cx}" cy="${cy}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <circle cx="${cx}" cy="${cy}" r="13" fill="#fff" stroke="#1a1a1a" stroke-width="2.5"/>
          <circle cx="${cx}" cy="${cy}" r="10" fill="#1a1a1a"/>
          <path d="M ${cx - 4} ${cy - 5} L ${cx - 1} ${cy - 1} L ${cx + 4} ${cy - 4}
                   L ${cx + 1} ${cy + 2} L ${cx + 5} ${cy + 5} L ${cx} ${cy + 3}
                   L ${cx - 5} ${cy + 6} L ${cx - 2} ${cy + 1} Z" fill="#fff"/>
          <circle cx="${cx + 5}" cy="${cy + 5}" r="1.5" fill="#fff"/>
        </g>
      `;
    }

    heartEye(cx, cy) {
      return `
        <g>
          <ellipse cx="${cx}" cy="${cy}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <path d="M ${cx} ${cy + 6}
                   C ${cx - 10} ${cy - 2}, ${cx - 12} ${cy - 8}, ${cx - 6} ${cy - 8}
                   C ${cx - 2} ${cy - 8}, ${cx} ${cy - 4}, ${cx} ${cy - 2}
                   C ${cx} ${cy - 4}, ${cx + 2} ${cy - 8}, ${cx + 6} ${cy - 8}
                   C ${cx + 12} ${cy - 8}, ${cx + 10} ${cy - 2}, ${cx} ${cy + 6} Z"
                fill="#FF3366" stroke="#1a1a1a" stroke-width="2"/>
          <circle cx="${cx - 4}" cy="${cy - 5}" r="1.5" fill="#fff"/>
        </g>
      `;
    }

    angryEye(cx, cy) {
      // Ceja gruesa inclinada hacia el centro + ojo entrecerrado
      const toward = cx < 100 ? 1 : -1; // inclina ceja hacia el centro de la cara
      return `
        <g>
          <ellipse cx="${cx}" cy="${cy}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <!-- ojo entrecerrado -->
          <path d="M ${cx - 11} ${cy + 1} Q ${cx} ${cy + 5} ${cx + 11} ${cy + 1} Q ${cx} ${cy - 3} ${cx - 11} ${cy + 1} Z"
                fill="#fff" stroke="#1a1a1a" stroke-width="2.5"/>
          <circle cx="${cx}" cy="${cy + 1}" r="4" fill="#1a1a1a"/>
          <!-- ceja gruesa inclinada -->
          <path d="M ${cx - 12 * toward} ${cy - 11} L ${cx + 12 * toward} ${cy - 4}"
                stroke="#1a1a1a" stroke-width="5" stroke-linecap="round"/>
        </g>
      `;
    }

    // Ojo en espiral estilo cómic para mareo/náuseas
    spiralEye(cx, cy) {
      return `
        <g>
          <ellipse cx="${cx}" cy="${cy}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <circle cx="${cx}" cy="${cy}" r="13" fill="#fff" stroke="#1a1a1a" stroke-width="2.5"/>
          <path d="M ${cx} ${cy}
                   m -1 0
                   a 2 2 0 1 1 2 0
                   a 4 4 0 1 1 -4 0
                   a 6 6 0 1 1 6 0
                   a 8 8 0 1 1 -8 0
                   a 10 10 0 1 1 10 0"
                fill="none" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>
        </g>
      `;
    }

    // Ojo cerrado cantando feliz (curva en forma de U invertida con pestañas)
    singingEye(cx, cy) {
      return `
        <g>
          <ellipse cx="${cx}" cy="${cy}" rx="18" ry="17" class="rigo-skin" stroke="#1a1a1a" stroke-width="3"/>
          <path d="M ${cx - 11} ${cy + 3} Q ${cx} ${cy - 8} ${cx + 11} ${cy + 3}"
                fill="none" stroke="#1a1a1a" stroke-width="3.5" stroke-linecap="round"/>
          <!-- pestañas alegres -->
          <path d="M ${cx - 11} ${cy + 3} L ${cx - 14} ${cy + 1}" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>
          <path d="M ${cx + 11} ${cy + 3} L ${cx + 14} ${cy + 1}" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>
          <path d="M ${cx} ${cy - 5} L ${cx} ${cy - 8}" stroke="#1a1a1a" stroke-width="2" stroke-linecap="round"/>
        </g>
      `;
    }

    // ======================= RENDER =======================
    render() {
      this.shadowRoot.innerHTML = `
        <style>
          :host {
            position: fixed;
            bottom: 20px;
            right: 20px;
            z-index: 9999;
            user-select: none;
            -webkit-user-select: none;
            -webkit-touch-callout: none;
            font-family: 'Comic Sans MS', 'Chalkboard SE', system-ui, sans-serif;
            touch-action: none;
          }
          #rigo-wrapper {
            position: relative;
            width: 140px;
            height: 140px;
            cursor: grab;
            filter: drop-shadow(3px 4px 0 rgba(0,0,0,0.25));
            transition: transform 0.2s cubic-bezier(0.34, 1.56, 0.64, 1);
          }
          #rigo-wrapper.dragging {
            cursor: grabbing;
            transition: none;
            filter: drop-shadow(6px 8px 0 rgba(0,0,0,0.35));
          }
          #rigo-wrapper:hover:not(.dragging) { transform: scale(1.08) rotate(-3deg); }
          #rigo-wrapper:active:not(.dragging) { transform: scale(0.95); }

          @keyframes popArtSkin {
            0%   { fill: #7ED957; }
            14%  { fill: #5AC850; }
            28%  { fill: #8FDB5F; }
            42%  { fill: #6BCF4A; }
            57%  { fill: #7FD856; }
            71%  { fill: #52BF48; }
            85%  { fill: #85D95B; }
            100% { fill: #7ED957; }
          }
          @keyframes popArtStroke {
            0%   { stroke: #7ED957; }
            14%  { stroke: #5AC850; }
            28%  { stroke: #8FDB5F; }
            42%  { stroke: #6BCF4A; }
            57%  { stroke: #7FD856; }
            71%  { stroke: #52BF48; }
            85%  { stroke: #85D95B; }
            100% { stroke: #7ED957; }
          }
          /* Cambio de color visible pero sutil (45s por ciclo completo) */
          .rigo-skin { animation: popArtSkin 45s infinite linear; fill: #7ED957; }
          .rigo-skin-stroke { animation: popArtStroke 45s infinite linear; stroke: #7ED957; }

          /* En modo angry la piel se vuelve roja y vibra */
          :host([data-emotion="angry"]) .rigo-skin {
            animation: angrySkin 0.4s infinite linear;
            fill: #E63946;
          }
          :host([data-emotion="angry"]) .rigo-skin-stroke {
            animation: angryStroke 0.4s infinite linear;
            stroke: #E63946;
          }
          @keyframes angrySkin {
            0%, 100% { fill: #E63946; }
            50%      { fill: #FF3333; }
          }
          @keyframes angryStroke {
            0%, 100% { stroke: #E63946; }
            50%      { stroke: #FF3333; }
          }

          @keyframes rigoBounce {
            0%, 100% { transform: translateY(0); }
            50%      { transform: translateY(-6px); }
          }
          @keyframes tailWave {
            0%, 100% { transform: rotate(0deg); }
            25%      { transform: rotate(-12deg); }
            75%      { transform: rotate(12deg); }
          }
          .rigo-tail-wave {
            animation: tailWave 0.8s ease-in-out infinite;
          }
          @keyframes rigoShake {
            0%, 100% { transform: translateX(0) rotate(0); }
            25%      { transform: translateX(-2px) rotate(-1deg); }
            75%      { transform: translateX(2px) rotate(1deg); }
          }
          :host([data-emotion="angry"]) #svg-container {
            animation: rigoShake 0.15s infinite linear;
          }

          /* Mareo: piel verdosa enferma + tambaleo lento */
          @keyframes dizzySkin {
            0%, 100% { fill: #A8C97A; }
            50%      { fill: #8FB868; }
          }
          @keyframes dizzyStroke {
            0%, 100% { stroke: #A8C97A; }
            50%      { stroke: #8FB868; }
          }
          :host([data-emotion="dizzy"]) .rigo-skin {
            animation: dizzySkin 1.2s infinite ease-in-out;
            fill: #A8C97A;
          }
          :host([data-emotion="dizzy"]) .rigo-skin-stroke {
            animation: dizzyStroke 1.2s infinite ease-in-out;
            stroke: #A8C97A;
          }
          @keyframes rigoSway {
            0%, 100% { transform: rotate(-4deg); }
            50%      { transform: rotate(4deg); }
          }
          :host([data-emotion="dizzy"]) #svg-container {
            animation: rigoSway 1.4s infinite ease-in-out;
          }
          @keyframes dizzyOrbit {
            0%   { transform: rotate(0deg); }
            100% { transform: rotate(360deg); }
          }
          .rigo-dizzy-orbit {
            animation: dizzyOrbit 2s infinite linear;
          }

          /* Musical: balanceo rítmico + notas flotando */
          @keyframes rigoSing {
            0%, 100% { transform: translateY(0) rotate(-2deg); }
            50%      { transform: translateY(-4px) rotate(2deg); }
          }
          :host([data-emotion="musical"]) #svg-container {
            animation: rigoSing 0.6s infinite ease-in-out;
          }
          @keyframes musicFloat {
            0%   { transform: translateY(0); opacity: 0.4; }
            50%  { transform: translateY(-8px); opacity: 1; }
            100% { transform: translateY(-16px); opacity: 0; }
          }
          .rigo-music-float {
            animation: musicFloat 1.8s infinite ease-out;
          }

          /* Anciano: movimiento muy lento y tembloroso */
          @keyframes elderTremble {
            0%, 100% { transform: translateX(0) translateY(0); }
            25%      { transform: translateX(0.5px) translateY(-0.3px); }
            50%      { transform: translateX(-0.3px) translateY(0.4px); }
            75%      { transform: translateX(0.4px) translateY(0.2px); }
          }
          :host([data-emotion="elder"]) #svg-container {
            animation: elderTremble 0.5s infinite linear;
          }
          @keyframes rigoIdle {
            0%, 100% { transform: translateY(0) rotate(0); }
            50%      { transform: translateY(-3px) rotate(1deg); }
          }

          #svg-container {
            width: 100%;
            height: 100%;
            animation: rigoIdle 4s ease-in-out infinite;
            pointer-events: none;
          }

          #speech-bubble {
            position: absolute;
            bottom: 110%;
            right: 10%;
            background: #fff;
            border: 3px solid #1a1a1a;
            border-radius: 18px;
            padding: 10px 14px;
            font-size: 13px;
            font-weight: bold;
            color: #1a1a1a;
            line-height: 1.3;
            width: max-content;
            max-width: 200px;
            text-align: center;
            box-shadow: 3px 4px 0 #1a1a1a;
            opacity: 0;
            transform: translateY(10px) scale(0.8);
            transition: opacity 0.25s, transform 0.25s cubic-bezier(0.34, 1.56, 0.64, 1);
            pointer-events: none;
            white-space: normal;
          }
          #speech-bubble::after {
            content: '';
            position: absolute;
            bottom: -10px;
            right: 25px;
            width: 0;
            height: 0;
            border: 10px solid transparent;
            border-top-color: #fff;
            border-bottom: 0;
          }
          #speech-bubble::before {
            content: '';
            position: absolute;
            bottom: -14px;
            right: 22px;
            width: 0;
            height: 0;
            border: 13px solid transparent;
            border-top-color: #1a1a1a;
            border-bottom: 0;
          }
          #speech-bubble.show {
            opacity: 1;
            transform: translateY(0) scale(1);
          }

          @media (max-width: 480px) {
            :host { bottom: 10px; right: 10px; }
            #rigo-wrapper { width: 110px; height: 110px; }
            #speech-bubble { font-size: 12px; max-width: 160px; }
          }
        </style>

        <div id="rigo-wrapper" translate="no">
          <div id="speech-bubble"></div>
          <div id="svg-container">${this.getSVG(this.currentEmotion)}</div>
        </div>
      `;
    }

    // ======================= INTERACCIONES =======================
    setupInteractions() {
      const wrapper = this.shadowRoot.getElementById('rigo-wrapper');

      const onDown = (e) => {
        this.isDragging = true;
        this.hasMoved = false;
        wrapper.classList.add('dragging');
        const point = e.touches ? e.touches[0] : e;
        const rect = this.getBoundingClientRect();
        this.dragOffsetX = point.clientX - rect.left;
        this.dragOffsetY = point.clientY - rect.top;
        e.preventDefault();
      };

      const onMove = (e) => {
        if (!this.isDragging) return;
        this.hasMoved = true;
        const point = e.touches ? e.touches[0] : e;
        const x = point.clientX - this.dragOffsetX;
        const y = point.clientY - this.dragOffsetY;
        const maxX = window.innerWidth - this.offsetWidth;
        const maxY = window.innerHeight - this.offsetHeight;
        this.style.left = Math.max(0, Math.min(x, maxX)) + 'px';
        this.style.top = Math.max(0, Math.min(y, maxY)) + 'px';
        this.style.right = 'auto';
        this.style.bottom = 'auto';
        e.preventDefault();
      };

      const onUp = () => {
        if (!this.isDragging) return;
        this.isDragging = false;
        wrapper.classList.remove('dragging');
        if (this.hasMoved) this.savePosition();
        else this.handleTap();
      };

      wrapper.addEventListener('mousedown', onDown);
      window.addEventListener('mousemove', onMove);
      window.addEventListener('mouseup', onUp);
      wrapper.addEventListener('touchstart', onDown, { passive: false });
      window.addEventListener('touchmove', onMove, { passive: false });
      window.addEventListener('touchend', onUp);
    }

    handleTap() {
      if (Math.random() < 0.008) {
        this.setEmotion('easterEgg');
        this.say("¡Modo POP ART desbloqueado!", 4000);
        
        // Limpiamos la emoción anterior si el usuario hizo clic rápido
        if (this.emotionTimer) clearTimeout(this.emotionTimer);
        this.emotionTimer = setTimeout(() => this.setEmotion('neutral'), 4000);
        return;
      }
      
      const msg = this.pickMessage(HINT_MESSAGES);
      const emotion = msg.emotion || 'sneaky';
      
      this.setEmotion(emotion);
      this.say(msg.text, 4000);
      
      this.dispatchEvent(new CustomEvent('rigo-hint-requested', {
        bubbles: true, composed: true,
        detail: { hint: msg.text, emotion }
      }));
      
      if (this.emotionTimer) clearTimeout(this.emotionTimer);
      this.emotionTimer = setTimeout(() => this.setEmotion('neutral'), 4000);
    }

    savePosition() {
      // Posición no persistida: cada recarga vuelve a la esquina por defecto.
      // Si el estudiante lo arrastra, se mantiene durante la sesión.
      try { localStorage.removeItem('rigo_pos'); } catch (e) { /* silencioso */ }
    }

    restorePosition() {
      try {
        const saved = localStorage.getItem('rigo_pos');
        if (!saved) return;
        const pos = JSON.parse(saved);
        if (pos.left && pos.top) {
          this.style.left = pos.left;
          this.style.top = pos.top;
          this.style.right = 'auto';
          this.style.bottom = 'auto';
        }
      } catch (e) { /* silencioso */ }
    }

    setEmotion(emotion) {
      if (!EMOTIONS.includes(emotion)) emotion = 'neutral';
      this.currentEmotion = emotion;
      this.setAttribute('data-emotion', emotion);
      const container = this.shadowRoot.getElementById('svg-container');
      if (container) container.innerHTML = this.getSVG(emotion);
    }

    say(text, duration = 4000) {
      const bubble = this.shadowRoot.getElementById('speech-bubble');
      if (!bubble) return;
      bubble.textContent = text;
      bubble.classList.add('show');
      if (this.bubbleTimer) clearTimeout(this.bubbleTimer);
      this.bubbleTimer = setTimeout(() => {
        bubble.classList.remove('show');
      }, duration);
    }

    startRandomMessages() {
      const schedule = () => {
        const delay = 45000 + Math.random() * 60000;
        this.randomTimer = setTimeout(() => {
          const bubble = this.shadowRoot.getElementById('speech-bubble');
          if (bubble && !bubble.classList.contains('show') && !this.isDragging) {
            const msg = this.pickMessage(RANDOM_MESSAGES);
            const prev = this.currentEmotion;
            const emotion = msg.emotion || 'happy';
            this.setEmotion(emotion);
            this.say(msg.text, 3500);
            setTimeout(() => this.setEmotion(prev), 3500);
          }
          schedule();
        }, delay);
      };
      schedule();
    }

    startBlinking() {
      this.blinkTimer = setInterval(() => {
        if (!['neutral', 'happy', 'welcome'].includes(this.currentEmotion)) return;
        const svg = this.shadowRoot.querySelector('#svg-container svg');
        if (!svg) return;
        const eyes = svg.querySelectorAll('circle[r="13"]');
        eyes.forEach(e => { e.style.transform = 'scaleY(0.1)'; e.style.transformOrigin = 'center'; });
        setTimeout(() => { eyes.forEach(e => e.style.transform = ''); }, 140);
      }, 4500 + Math.random() * 3000);
    }

    // ======================= API PÚBLICA =======================
    setGrade(grade) {
      this.grade = grade || null;
      this.setAttribute('grade', grade || '');
    }

    welcome() {
      this.setEmotion('welcome');
      this.say("¡Hola! Soy Rigo, tu compañero de Cívica. Escribe tu NIE para empezar", 7000);
    }

    loginSuccess(name) {
      this.setEmotion('excited');
      const msg = name ? `¡Hola ${name}! ¡Listos para reflexionar!` : "¡Bienvenido! ¡A construir ciudadanía!";
      this.say(msg, 4000);
      setTimeout(() => this.setEmotion('neutral'), 4000);
    }

    inviteGame() {
      this.setEmotion('game');
      this.say("¿Qué tal un juego de repaso? 🎮", 5000);
    }

    cheer() {
      this.setEmotion('excited');
      const msg = CHEER_MESSAGES[Math.floor(Math.random() * CHEER_MESSAGES.length)];
      this.say(msg, 2500);
      
      if (this.emotionTimer) clearTimeout(this.emotionTimer);
      this.emotionTimer = setTimeout(() => this.setEmotion('happy'), 2500);
    }

    comfort() {
      this.setEmotion('sad');
      const msg = COMFORT_MESSAGES[Math.floor(Math.random() * COMFORT_MESSAGES.length)];
      this.say(msg, 3000);
      setTimeout(() => this.setEmotion('neutral'), 3000);
    }
  }

  customElements.define('rigo-mascot', RigoMascot);

  window.addEventListener('DOMContentLoaded', () => {
    window.rigo = document.querySelector('rigo-mascot');
  });
})();
