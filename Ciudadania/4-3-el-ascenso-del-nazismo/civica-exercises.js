    ...
    items.forEach((it, i) => {
      ...
      row.innerHTML = `
        ...
        <div class="cv-ta-meta" data-meta="${i}"
          ...
          <span class="cv-ta-min">Mínimo recomendado: ${minLen}</span>
        </div>
      `;
      wrap.appendChild(row);
  ...
  reg('cv_table', function (ctx) {
    ...
    function buildCellContent(td, cell, rIdx, cIdx) {
      if (cell.type === 'text') {
        ...
        td.innerHTML = `<input type="text" class="civica-input-allow-copy"
      ...
      cellRefs.forEach((rowRefs, rIdx) => {
        ...
        rowRefs.forEach(({ td, cell }, cIdx) => {
          ...
          if (cell.type === 'input') {
            ...
            const ok = C.normalize(val) === C.normalize(cell.answer || '');
            if (ok) correct++;
            if (!ctx.examMode) {
              inp.classList.toggle('correct', ok);
              inp.classList.toggle('wrong', !ok);
            ...
            inp.disabled = true;
            details.push(`F${rIdx+1}C${cIdx+1}: ${ok ? 'OK' : 'X (esperado: '+cell.answer+', tuyo: '+(val||'vacio')+')'}`);
            rowSnap.push(val);
