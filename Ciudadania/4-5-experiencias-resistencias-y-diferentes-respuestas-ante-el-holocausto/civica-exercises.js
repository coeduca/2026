/**
 * CIVICA Framework v1 - Exercises
 * 13 tipos heredados de inglés + 2 nuevos para Ciudadanía y Valores.
 * Depende de civica-core.js.
 *
...
(function (global) {
  'use strict';
  if (!global.CIVICA) {
    console.error('civica-core.js debe cargarse antes que civica-exercises.js');
    return;
  }
  const C = global.CIVICA;
  const reg = (type, fn) => C.registerExercise(type, fn);

  // Las respuestas abiertas de Ciudadanía y Valores se validan por desarrollo,
  // no por coincidencia literal con una respuesta generada por IA.
  // "Más de 15 caracteres" = 16 caracteres como mínimo.
  const CIVICA_OPEN_ANSWER_MIN_LENGTH = 16;
  const isDevelopedOpenAnswer = (value) => (value || '').trim().length >= CIVICA_OPEN_ANSWER_MIN_LENGTH;

  // Inyectar estilos auxiliares una sola vez (placeholder de reorder_sentences)
  (function injectAuxStyles() {
    if (document.getElementById('civica-aux-styles')) return;
    const style = document.createElement('style');
    style.id = 'civica-aux-styles';
    style.textContent =
  ...
  function feedback(target, ok, ctx) {
    ...
    span.className = 'civica-feedback ' + (ok ? 'correct' : 'wrong');
  ...
  reg('matchlines', function (ctx) {
    ...
    wrap.innerHTML = `
      <div style="position:relative; padding: 20px 0; width: 100%;">
        ...
        <div style="display:flex; justify-content:space-between; gap:20px; position:relative; z-index:2; padding: 0 5px;">
          <div id="ml-left-${ctx.exerciseId}" style="display:flex; flex-direction:column; justify-content:center; gap:15px; flex:1; max-width:48%;"></div>
          <div id="ml-right-${ctx.exerciseId}" style="display:flex; flex-direction:column; justify-content:center; gap:15px; flex:1; max-width:48%;"></div>
        </div>
      </div>
    `;
    ctx.container.appendChild(wrap);
    ...
    function makeItem(item, side) {
      ...
      el.style.cssText = `background:${bgGrad}; border:1.5px solid ${brdColor}; border-radius:12px;
        padding:12px 14px; font-weight:600; font-size:14px; line-height:1.4; cursor:pointer; box-sizing: border-box;
        color: var(--civica-text); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px);
        text-align:center; box-shadow:0 4px 12px rgba(56,38,130,0.10); transition:transform 0.18s, box-shadow 0.18s;
        width:100%; word-wrap:break-word; position:relative; z-index:2;`;
    ...
    // Dibujamos las líneas una vez que el DOM ya acomodó los tamaños
    setTimeout(() => {
      redrawAll();
    }, 100);
  ...
  reg('dragbank', function (ctx) {
    ...
    const wrap = document.createElement('div');
    wrap.innerHTML = `
      <div id="db-bank-${ctx.exerciseId}" class="civica-panel-soft" style="display:flex;flex-wrap:wrap;gap:8px;
    ...
    function makeWord(w) {
      const el = document.createElement('div');
      el.textContent = w;
      el.className = 'db-word civica-chip civica-chip-accent';
      el.dataset.word = w;
    ...
    sentences.forEach((s, i) => {
      ...
      const slot = `<span class="db-slot civica-slot" data-i="${i}" data-answer="${C.escapeHTML(s.answer)}"></span>`;
    ...
    function attachDrag(el) {
      C.makeDraggable(el, {
        onMove: ({ under }) => {
          document.querySelectorAll('.db-slot').forEach(s => s.classList.remove('civica-drop-hover'));
          const slot = under && under.closest && under.closest('.db-slot');
          if (slot) slot.classList.add('civica-drop-hover');
        },
        onDrop: ({ under }) => {
          document.querySelectorAll('.db-slot').forEach(s => s.classList.remove('civica-drop-hover'));
    ...
    function check() {
      ...
      sentEl.querySelectorAll('.db-slot').forEach(slot => {
        ...
        if (w && C.normalize(w.textContent) === C.normalize(ans)) {
          correct++;
          if (!ctx.examMode) {
            slot.style.borderColor = 'var(--civica-success)';
            slot.style.background = 'rgba(56,178,106,0.12)';
          }
          details.push(ans + ': OK');
        } else {
          if (!ctx.examMode) {
            slot.style.borderColor = w ? 'var(--civica-error)' : 'rgba(124,92,255,0.4)';
  ...
  reg('reorder_sentences', function (ctx) {
    ...
    items.forEach((it, i) => {
      ...
      rowWrap.className = 'civica-panel';
      rowWrap.style.cssText = 'margin:14px 0;';

      const header = document.createElement('div');
      header.style.cssText = 'font-weight:700;margin-bottom:12px;color:var(--civica-text);';
      ...
      if (it.es) {
        ...
        hint.style.cssText = 'margin:0 0 10px 0;padding:8px 14px;background:rgba(246,185,59,0.12);' +
          'border:1px solid rgba(246,185,59,0.4);border-left:3px solid var(--civica-amber);' +
          'border-radius:8px;font-style:italic;font-size:14px;' +
          'color:var(--civica-text-soft);';
      ...
      it.shuffled.forEach((w) => {
        ...
        chip.textContent = w;
        chip.dataset.word = w;
        chip.className = 'civica-chip civica-chip-info';
        ...
        C.makeDraggable(chip, {
          ...
          onMove: ({ under }) => {
            ...
            if (hoverBox) hoverBox.style.borderColor = 'var(--civica-primary)';
          },
    ...
    btn.addEventListener('click', () => {
      ...
      items.forEach((it, i) => {
        ...
        if (!ctx.examMode) {
          ...
          targetBox.style.borderColor = ok ? 'var(--civica-success)' : 'var(--civica-error)';
        }
  ...
  reg('reorder_letters', function (ctx) {
    ...
    items.forEach((it, i) => {
      const row = document.createElement('div');
      row.className = 'civica-panel';
      row.style.cssText = 'margin:12px 0;';
      
      // Envolvemos las letras y el input en un div con flexbox
      row.innerHTML = `
        <div style="font-weight:700;margin-bottom:10px;color:var(--civica-text);">${i + 1}. ${C.escapeHTML(it.hint || 'Forma la palabra')}</div>
        <div style="display:flex; flex-wrap:wrap; align-items:center; gap:12px;">
          <div style="font-family:'JetBrains Mono', 'Courier New', monospace;font-size:22px;letter-spacing:6px;font-weight:800;
               ...
               color:var(--civica-primary);">
            ${it.shuffled}
          </div>
          <input type="text" class="civica-input rl-input" data-i="${i}" placeholder="Escribe la palabra"
 ...
 // =====================================================================
  ...
  reg('emojiphrase', function (ctx) {
    ...
    items.forEach((it, i) => {
      ...
      card.style.cssText = `
        position: relative;
        background: ${bgGrad};
        border: 1px solid var(--civica-glass-brd);
        border-radius: var(--civica-radius);
        padding: 26px 18px 18px;
        box-shadow: var(--civica-shadow-sm);
        text-align: center;
        display: flex;
      ...
      card.innerHTML = `
        <div style="position: absolute; top: -12px; left: -12px; background: linear-gradient(135deg, var(--civica-primary) 0%, var(--civica-accent) 100%); color: #fff; border: 1px solid rgba(255,255,255,0.4); border-radius: 50%; width: 34px; height: 34px; display: grid; place-items: center; font-weight: 700; font-size: 15px; box-shadow: 0 4px 12px rgba(124,92,255,0.4); z-index: 2;">
          ${i + 1}
        </div>
        <div style="font-size: 56px; line-height: 1.2; margin-bottom: 14px;">
          ${C.escapeHTML(it.emojis)}
        </div>
        <div style="font-weight: 700; font-size: 12px; margin-bottom: 10px; color: var(--civica-text-soft); text-transform: uppercase; letter-spacing: 1px;">
          Escribe tu respuesta
        </div>
        <input type="text" class="civica-input ep-input" data-i="${i}"
               style="width: 100%; text-align: center; font-size: 16px;" autocomplete="off" placeholder="">
    ...
    items.forEach((it, i) => {
      ...
      let optsHTML = it.options.map((opt, j) => `
        <label class="mc-lbl-${ctx.exerciseId}-${i}" style="display:flex; align-items:center; background:rgba(255,255,255,0.7); 
               ...
               font-weight:500; cursor:pointer; box-shadow:0 2px 8px rgba(56,38,130,0.06); 
               color:var(--civica-text-soft);
          ...
          <input type="radio" name="mc-${ctx.exerciseId}-${i}" value="${j}" 
                 style="margin-right:12px; cursor:pointer; transform: scale(1.2); accent-color: var(--civica-primary);">
          <span style="flex:1; line-height:1.4;">${C.escapeHTML(opt)}</span>
        </label>
      `).join('');
      
      row.innerHTML = `<div style="font-weight:700; margin-bottom:12px; font-size:16px; color:var(--civica-text);">${i + 1}. ${C.escapeHTML(it.q)}</div>${optsHTML}`;
      wrap.appendChild(row);
      ...
      inputs.forEach(inp => {
        inp.addEventListener('change', () => {
          ...
          if (inp.checked) {
            ...
            activeLabel.style.borderColor = 'var(--civica-primary)';
        ...
        if (ctx.examMode) {
          ...
          wrap.querySelectorAll(`input[name="mc-${ctx.exerciseId}-${i}"]`).forEach(r => {
            ...
            if (+r.value === it.answer) {
              ...
              lbl.style.borderColor = 'var(--civica-success)';
            } else if (r.checked) {
              lbl.style.background = 'rgba(230,57,70,0.10)';
              lbl.style.borderColor = 'var(--civica-error)';
            } else {
  ...
  reg('spoterror', function (ctx) {
    ...
    items.forEach((it, i) => {
      ...
      const wordSpans = it.words.map((w, j) => `
        <span class="se-word se-word-${ctx.exerciseId}-${i}" data-i="${i}" data-j="${j}"
              style="display:inline-block; background:rgba(255,255,255,0.9); border:1.5px solid rgba(124,92,255,0.20);
                     ...
                     color:var(--civica-text); position:relative;">
          ${C.escapeHTML(w)}
        </span>
      `).join('');
      ...
      row.innerHTML = `
        <div style="font-weight:700; font-size:16px; margin-bottom:4px; color:var(--civica-text);">
        ...
        <div class="se-fix-feedback" data-fix-i="${i}" role="status"
             style="display:none; margin-top:12px; padding:10px 14px; align-items:center; justify-content:center;
                    gap:8px; flex-wrap:wrap; background:rgba(56,178,106,0.14);
                    border:1.5px solid var(--civica-success); border-radius:12px;
                    color:var(--civica-text); font-size:16px; font-weight:700;"></div>`;
      wrap.appendChild(row);
    });
    ...
    wrap.addEventListener('click', e => {
      ...
      w.style.borderColor = 'var(--civica-primary)';
    ...
    wrap.appendChild(gradeButton(() => {
      ...
      items.forEach((it, i) => {
        ...
        const fixFeedback = wrap.querySelector(`.se-fix-feedback[data-fix-i="${i}"]`);

        if (fixFeedback && it.fix) {
          const wrongWord = it.words[it.errorIndex] || '';
          fixFeedback.innerHTML = `
            <span>Corrección:</span>
            <s>${C.escapeHTML(wrongWord)}</s>
            <span aria-hidden="true">→</span>
            <strong>${C.escapeHTML(it.fix)}</strong>`;
          fixFeedback.style.display = 'flex';
        }
        ...
        if (ctx.examMode) {
          ...
          allWords.forEach(w => {
            ...
            if (j === it.errorIndex) {
              ...
              w.style.borderColor = 'var(--civica-success)';
              w.style.color = 'var(--civica-text)';
              w.style.boxShadow = '0 4px 12px rgba(56,178,106,0.25)';
            
              if (it.fix && !fixFeedback) {
                const fixBadge = document.createElement('div');
                // Bocadillo moderno con la corrección
                fixBadge.innerHTML = `
                  <div style="background:linear-gradient(135deg, var(--civica-success) 0%, var(--civica-cyan) 100%); color:#fff; font-size:13px; padding:6px 14px; border-radius:50px; 
              ...
              w.style.color = 'var(--civica-text-muted)';
  ...
  // =====================================================================
  // 10. CATEGORIZE (Clasificar con ajuste post-revisión)
  // =====================================================================
  reg('categorize', function (ctx) {
    ...
    const wrap = document.createElement('div');
    wrap.innerHTML = `
      <div id="cat-bank-${ctx.exerciseId}" style="display:flex; flex-wrap:wrap; gap:8px;
           margin-bottom:16px; min-height:50px; padding:12px; border: 1.5px dashed var(--civica-divider-2); border-radius: var(--civica-radius); background: rgba(255,255,255,0.5);"></div>
      <div style="display:grid; grid-template-columns:repeat(${cats.length}, 1fr); gap:12px;">
        ${cats.map((c, i) => `
          <div class="cat-col civica-panel" data-cat="${i}" style="padding:12px; min-height:140px; display:flex; flex-direction:column;">
            <h4 style="margin:0 0 12px; text-align:center; background:linear-gradient(135deg, var(--civica-primary) 0%, var(--civica-accent) 100%); color:#fff; border:1px solid rgba(255,255,255,0.3);
            ...
            <div class="cat-drop" data-cat="${i}" style="flex:1; min-height:80px; display:flex; flex-direction:column; gap:8px;"></div>
          </div>
        `).join('')}
      </div>
    `;
    ...
    items.forEach((it, idx) => {
      ...
      // Función para resetear el estilo al color original (píldora neutra)
      const resetStyle = () => {
        chip.style.background = 'var(--civica-surface)';
        chip.style.borderColor = 'var(--civica-divider-2)';
        chip.style.color = 'var(--civica-text-soft)';
        chip.style.boxShadow = 'var(--civica-shadow-sm)';
      };

      chip.style.cssText = `
        display: inline-block; padding: 8px 16px; border-radius: 50px;
        font-size: 13.5px; font-weight: 600; cursor: grab; user-select: none;
      ...
      C.makeDraggable(chip, {
        onPickup: () => {
          // En cuanto el alumno lo levanta para moverlo, limpiamos el verde/rojo previo
          resetStyle();
        },
        onDrop: ({ under }) => {
    ...
    wrap.appendChild(gradeButton(() => {
      ...
      items.forEach((it, idx) => {
        ...
        if (parent && +parent.dataset.cat === it.cat) {
          correct++;
          if (!ctx.examMode) {
            chip.style.background = 'rgba(56,178,106,0.15)'; // Verde suave
            chip.style.borderColor = 'var(--civica-success)';
            chip.style.color = 'var(--civica-text)';
          }
          details.push(it.text + ' -> ' + cats[it.cat] + ': OK');
        } else {
          if (!ctx.examMode) {
            chip.style.background = 'rgba(230,57,70,0.1)'; // Rojo suave
            chip.style.borderColor = 'var(--civica-error)';
            chip.style.color = 'var(--civica-text)';
          }
          details.push(it.text + ' -> ' + cats[it.cat] + ': ' +
            (parent ? 'incorrecto' : 'sin colocar'));
        }
      });
  ...
  reg('dropdown', function (ctx) {
    ...
    items.forEach((it, i) => {
      const row = document.createElement('div');
      row.className = 'civica-panel';
      row.style.cssText = 'margin:10px 0;font-size:16px;font-weight:500;color:var(--civica-text);';
      const opts = it.options.map((o, j) =>
        `<option value="${j}">${C.escapeHTML(o)}</option>`).join('');
      row.innerHTML = `${i + 1}. ${C.escapeHTML(it.before || '')}
        <select class="civica-input dd-sel" data-i="${i}" style="font-size:15px;padding:4px 8px;">
  ...
  reg('matchimage', function (ctx) {
    ...
    const wrap = document.createElement('div');
    wrap.innerHTML = `
      <div id="mi-bank-${ctx.exerciseId}" class="civica-panel-soft" style="display:flex;flex-wrap:wrap;gap:8px;
           margin-bottom:14px;"></div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:10px;">
        ${pairs.map(p => `
          <div class="mi-target civica-panel" data-idx="${p.idx}"
    ...
    shuffled.forEach(p => {
      ...
      C.makeDraggable(chip, {
        // --- AGREGA ESTO ---
        onMove: ({ under }) => {
          document.querySelectorAll('.mi-slot').forEach(s => s.classList.remove('civica-drop-hover'));
  ...
  reg('fillblank', function (ctx) {
    ...
    items.forEach((it, i) => {
      ...
      // Dejamos el número en negrita y color oscuro
      let html = `<span style="font-weight:700; color:var(--civica-text); margin-right:4px;">${i + 1}.</span> `;
      
      parts.forEach((p, j) => {
        html += C.escapeHTML(p);
        if (j < parts.length - 1) {
          const ans = answers[j] || '';
          // Le añadimos un font-weight:600 al input para que lo que escriba el alumno sí destaque
          html += `<input type="text" class="civica-input fb-input" data-i="${i}" data-j="${j}"
                          data-answer="${C.escapeHTML(ans)}"
                          style="width:${Math.max(70, ans.length * 14)}px; margin:0 6px; font-weight:600; text-align:center;"
                          autocomplete="off">`;
        }
      });
      const row = document.createElement('div');
      // Quitamos el font-weight:bold general, usamos 500 y un color más suave
      row.style.cssText = 'margin:16px 0; font-size:16px; font-weight:500; line-height:2; color:var(--civica-text-soft);';
      row.innerHTML = html;
      wrap.appendChild(row);
    });
  ...
  // =====================================================================
  // 14. TRUEFALSE (verdadero / falso - diseño en columnas)
  // =====================================================================
  reg('truefalse', function (ctx) {
    ...
    items.forEach((it, i) => {
      const row = document.createElement('div');
      row.className = 'civica-panel';
      // Usamos flexbox para alinear el texto a la izquierda y los botones a la derecha
      row.style.cssText = 'margin:14px 0; display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; gap:20px; padding:16px 20px;';
      
      row.innerHTML = `
        <div style="flex:1; min-width:200px; font-weight:500; font-size:15.5px; color:var(--civica-text-soft); line-height:1.5;">
          <span style="font-weight:700; color:var(--civica-text); margin-right:4px;">${i + 1}.</span> 
          ${C.escapeHTML(it.statement)}
        </div>
        <div style="display:flex; flex-direction:column; gap:10px; flex-shrink:0; min-width:140px;">
          <button class="civica-btn tf-btn" data-i="${i}" data-v="true"
                  style="background:rgba(255,255,255,0.7); border:1.5px solid rgba(56,178,106,0.3); color:var(--civica-text-soft); border-radius:10px; padding:8px 12px; font-size:13.5px; transition:all 0.2s ease;">
            Verdadero
          </button>
          <button class="civica-btn tf-btn" data-i="${i}" data-v="false"
                  style="background:rgba(255,255,255,0.7); border:1.5px solid rgba(230,57,70,0.3); color:var(--civica-text-soft); border-radius:10px; padding:8px 12px; font-size:13.5px; transition:all 0.2s ease;">
            Falso
          </button>
        </div>
      `;
    ...
    wrap.addEventListener('click', e => {
      ...
      const trueBtn = wrap.querySelector(`.tf-btn[data-i="${i}"][data-v="true"]`);
      const falseBtn = wrap.querySelector(`.tf-btn[data-i="${i}"][data-v="false"]`);
      
      // Resetear estilos de ambos botones a su estado inicial
      trueBtn.style.background = 'rgba(255,255,255,0.7)';
      trueBtn.style.color = 'var(--civica-text-soft)';
      trueBtn.style.borderColor = 'rgba(56,178,106,0.3)';
      trueBtn.style.boxShadow = 'none';
      trueBtn.style.fontWeight = '600';

      falseBtn.style.background = 'rgba(255,255,255,0.7)';
      falseBtn.style.color = 'var(--civica-text-soft)';
      falseBtn.style.borderColor = 'rgba(230,57,70,0.3)';
      falseBtn.style.boxShadow = 'none';
      falseBtn.style.fontWeight = '600';

      // Aplicar estilos "activos" al botón seleccionado
      if (selections[i]) {
        trueBtn.style.background = 'var(--civica-success)';
        trueBtn.style.color = '#fff';
        trueBtn.style.borderColor = 'var(--civica-success)';
        trueBtn.style.boxShadow = '0 4px 12px rgba(56,178,106,0.25)';
        trueBtn.style.fontWeight = '700';
      } else {
        falseBtn.style.background = 'var(--civica-error)';
        falseBtn.style.color = '#fff';
        falseBtn.style.borderColor = 'var(--civica-error)';
        falseBtn.style.boxShadow = '0 4px 12px rgba(230,57,70,0.25)';
    ...
    wrap.appendChild(gradeButton(() => {
      ...
      items.forEach((it, i) => {
        ...
        // Deshabilitar los botones para que no se puedan cambiar tras revisar
        wrap.querySelectorAll(`.tf-btn[data-i="${i}"]`).forEach(btn => {
          btn.style.pointerEvents = 'none';
          if (btn.style.color !== 'rgb(255, 255, 255)') { // Si no está seleccionado
            btn.style.opacity = '0.5';
          }
        });

        details.push(it.statement.substring(0, 40) + ': ' +
          (ok ? 'OK' : 'X (correcta: ' + (it.answer ? 'Verdadero' : 'Falso') + ')'));
      });
      ctx.recordAnswer(correct, items.length, details);
      if (!ctx.examMode) {
        if (correct === items.length) ctx.cheer(); else ctx.comfort();
  ...
  // =====================================================================
  // WORDSELECT (seleccionar palabras clave dentro de un texto)
  // =====================================================================
  reg('wordselect', function (ctx) {
    const items = ctx.data || [];
    const itemState = [];
    ...
    counterEl.className = 'wsel-counter-global';
    counterEl.style.cssText = 'margin:0 0 10px;font-size:13px;color:#555;font-weight:bold;text-align:right;';

    items.forEach((it, i) => {
      const tokens = [];
      const re = /\[([^\]]+)\]|([^\[\]]+)/g;
      let m;
      while ((m = re.exec(String(it.text || ''))) !== null) {
        const isTarget = m[1] != null;
        const chunk = isTarget ? m[1] : m[2];
        const pieces = chunk.match(/[\p{L}\p{N}'’\-]+|\s+|[^\p{L}\p{N}\s]+/gu) || [chunk];
        pieces.forEach(piece => {
          const isWord = /[\p{L}\p{N}]/u.test(piece);
          tokens.push({ text: piece, isTarget: isWord ? isTarget : false, picked: false, sep: !isWord });
        });
      }
      const targetCount = tokens.filter(t => t.isTarget).length;
      itemState.push({ tokens, targetCount });

      const block = document.createElement('div');
      block.className = 'wsel-block civica-panel';
      block.dataset.i = i;
      block.style.cssText = 'margin:14px 0;padding:14px;';
      const hintHtml = it.hint
        ? '<div style="font-weight:900;text-transform:uppercase;font-size:12px;background:#FFD700;border:2px solid #1a1a1a;border-radius:50px;padding:4px 12px;display:inline-block;margin-bottom:10px;letter-spacing:0.5px;">' + C.escapeHTML(it.hint) + '</div>'
        : '';
      const wordsHtml = tokens.map((t, ti) => {
        if (t.sep) return C.escapeHTML(t.text).replace(/\n/g, '<br>');
        return '<span class="wsel-word" data-i="' + i + '" data-ti="' + ti + '" '
          + 'style="cursor:pointer;border-radius:4px;padding:1px 2px;transition:background 0.15s;">'
          + C.escapeHTML(t.text) + '</span>';
      }).join('');
      block.innerHTML = hintHtml + '<div style="line-height:1.7;font-size:16px;word-wrap:break-word;">' + wordsHtml + '</div>';
      wrap.appendChild(block);
    });

    const totalTargets = itemState.reduce((acc, s) => acc + s.targetCount, 0);
    function refreshCounter() {
      const sel = itemState.reduce((acc, s) =>
        acc + s.tokens.filter(t => t.picked && t.isTarget).length, 0);
      counterEl.textContent = 'Seleccionadas: ' + sel + ' / ' + totalTargets;
    }
    refreshCounter();
    if (totalTargets > 0) wrap.appendChild(counterEl);

    wrap.addEventListener('click', e => {
      const w = e.target.closest('.wsel-word');
      if (!w) return;
      const i = +w.dataset.i;
      const ti = +w.dataset.ti;
      const tok = itemState[i].tokens[ti];
      tok.picked = !tok.picked;
      if (tok.picked) {
        w.style.background = '#FFD700';
        w.style.fontWeight = '900';
        w.style.boxShadow = 'inset 0 -3px 0 #1a1a1a';
        if (!ctx.examMode) { if (tok.isTarget) ctx.cheer(); else ctx.comfort(); }
      } else {
        w.style.background = '';
        w.style.fontWeight = '';
        w.style.boxShadow = '';
    ...
    wrap.appendChild(gradeButton(() => {
      ...
      itemState.forEach((s, i) => {
        s.tokens.forEach((t, ti) => {
          if (t.sep) return;
          if (t.isTarget) total++;
          const w = wrap.querySelector('.wsel-word[data-i="' + i + '"][data-ti="' + ti + '"]');
          if (t.isTarget && t.picked) {
            correctTargets++;
            if (w) { w.style.background = '#4CAF50'; w.style.color = '#fff'; w.style.fontWeight = '900'; w.style.boxShadow = ''; }
          } else if (t.isTarget && !t.picked) {
            if (w) { w.style.background = '#FFE0E0'; w.style.boxShadow = 'inset 0 -2px 0 #E63946'; }
          } else if (!t.isTarget && t.picked) {
            wrongPicks++;
            if (w) { w.style.background = '#E63946'; w.style.color = '#fff'; w.style.textDecoration = 'line-through'; w.style.boxShadow = ''; }
          }
        });
        details.push('Texto ' + (i + 1) + ': ' + correctTargets + '/' + total + ' correctas, ' + wrongPicks + ' erróneas');
      });
      const score = Math.max(0, correctTargets - wrongPicks);
      ctx.recordAnswer(score, total || 1, details);
      if (!ctx.examMode) {
        if (score === total && wrongPicks === 0) ctx.cheer(); else ctx.comfort();
      }
    }, ctx));
    ctx.container.appendChild(wrap);
  });
  ...
  // =====================================================================
  // MEMORY (memorama / encuentra la pareja)
  // =====================================================================
  (function injectMemoryStyles() {
    if (document.getElementById('civica-memory-styles')) return;
    const s = document.createElement('style');
    s.id = 'civica-memory-styles';
    s.textContent = ''
      + '@keyframes memPop {0%{transform:scale(1)}40%{transform:scale(1.12) rotate(-3deg)}100%{transform:scale(1)}}'
      + '@keyframes memBurst {0%{opacity:1;transform:scale(0.4)}100%{opacity:0;transform:scale(1.6)}}'
      + '@keyframes memConfetti {0%{opacity:1;transform:translate(0,0) rotate(0)}100%{opacity:0;transform:translate(var(--cx,0),var(--cy,40px)) rotate(var(--cr,180deg))}}'
      + '.mem-card .mem-inner{position:absolute;inset:0;transition:transform 0.5s;transform-style:preserve-3d}'
      + '.mem-card .mem-face{position:absolute;inset:0;backface-visibility:hidden;-webkit-backface-visibility:hidden;border-radius:11px;display:grid;place-items:center}'
      + '.mem-card .mem-front{transform:rotateY(180deg)}'
      + '.mem-card.is-flipped .mem-inner{transform:rotateY(180deg)}'
      + '.mem-card.mem-matched{animation:memPop 0.6s ease}';
    document.head.appendChild(s);
  })();

  reg('memory', function (ctx) {
    const pairs = (ctx.data || []).filter(p => p && (p.left || p.right));
    if (!pairs.length) {
      const empty = document.createElement('div');
      empty.style.cssText = 'padding:14px;font-style:italic;color:#666;';
      empty.textContent = 'Sin pares definidos.';
    ...
    let cards = [];
    pairs.forEach((p, i) => {
      cards.push({ pairId: i, text: p.left || '?' });
      cards.push({ pairId: i, text: p.right || '?' });
    });
    cards = C.shuffle(cards);

    const total = cards.length;
    const cols = total <= 6 ? Math.min(3, total)
               : total <= 12 ? 4
               : total <= 20 ? 5 : 6;

    const wrap = document.createElement('div');
    wrap.innerHTML =
      '<div class="mem-info" style="text-align:center;margin-bottom:12px;font-weight:900;font-size:14px;background:#fff;border:3px solid #1a1a1a;border-radius:50px;padding:6px 14px;display:inline-block;left:50%;position:relative;transform:translateX(-50%);box-shadow:2px 2px 0 #1a1a1a;">'
      +   'Movimientos: <span class="mem-moves">0</span> &nbsp;·&nbsp; Pares: <span class="mem-found">0</span> / ' + pairs.length
      + '</div>'
      + '<div id="mem-grid-' + ctx.exerciseId + '" class="mem-grid" '
      +   'style="display:grid;grid-template-columns:repeat(' + cols + ',minmax(0,1fr));gap:10px;max-width:560px;margin:0 auto;"></div>';
    ctx.container.appendChild(wrap);

    const gridEl = wrap.querySelector('#mem-grid-' + ctx.exerciseId);
    const infoMoves = wrap.querySelector('.mem-moves');
    const infoFound = wrap.querySelector('.mem-found');
    ...
    const palette = ['#FF6B9D', '#4FC3F7', '#FFD700', '#9B5DE5', '#FF9F1C', '#43B069', '#E63946', '#29B6F6'];
    ...
    cards.forEach((card, idx) => {
      ...
      el.style.cssText = 'position:relative;aspect-ratio:1/1;border:3px solid #1a1a1a;border-radius:14px;cursor:pointer;background:transparent;box-shadow:3px 3px 0 #1a1a1a;user-select:none;-webkit-user-select:none;-webkit-tap-highlight-color:transparent;';
      el.innerHTML =
        '<div class="mem-inner">'
        +   '<div class="mem-face mem-back" style="background:repeating-linear-gradient(45deg,#FFD700 0 10px,#FF6B9D 10px 20px);color:#1a1a1a;font-weight:900;font-size:30px;text-shadow:2px 2px 0 #fff;">?</div>'
        +   '<div class="mem-face mem-front" style="background:' + palette[card.pairId % palette.length] + ';color:#fff;font-weight:900;text-shadow:1px 1px 0 #1a1a1a;font-size:clamp(14px,4.2vw,22px);text-align:center;padding:6px;word-break:break-word;line-height:1.15;">' + C.escapeHTML(card.text) + '</div>'
        + '</div>';
      gridEl.appendChild(el);
    });

    function flipFront(el) { el.classList.add('is-flipped'); }
    function flipBack(el) { el.classList.remove('is-flipped'); }

    function popMatch(elA, elB) {
      [elA, elB].forEach(el => {
        el.classList.add('mem-matched');
        el.style.pointerEvents = 'none';
        const burst = document.createElement('div');
        burst.style.cssText = 'position:absolute;inset:-6px;border-radius:18px;background:radial-gradient(circle,rgba(255,215,0,0.65) 0%,transparent 70%);pointer-events:none;animation:memBurst 0.7s ease forwards;z-index:2;';
        el.appendChild(burst);
        const colors = ['#FFD700', '#FF6B9D', '#4FC3F7', '#9B5DE5', '#43B069'];
        for (let k = 0; k < 6; k++) {
          const c = document.createElement('div');
          const dx = (Math.random() * 80 - 40) + 'px';
          const dy = (Math.random() * 60 + 20) + 'px';
          const dr = (Math.random() * 360 - 180) + 'deg';
          c.style.cssText = 'position:absolute;left:50%;top:50%;width:8px;height:8px;background:' + colors[k % colors.length] + ';border:1.5px solid #1a1a1a;border-radius:2px;pointer-events:none;animation:memConfetti 0.8s ease forwards;--cx:' + dx + ';--cy:' + dy + ';--cr:' + dr + ';z-index:3;';
          el.appendChild(c);
        }
        setTimeout(() => {
          burst.remove();
          el.querySelectorAll('div[style*="memConfetti"]').forEach(n => n.remove());
        }, 900);
      });
    }
    ...
    gridEl.addEventListener('click', e => {
      ...
      if (!card || card.classList.contains('mem-matched')) return;
      ...
      const ok = firstCard.dataset.pair === card.dataset.pair;
      if (ok) {
        popMatch(firstCard, card);
        foundPairs++;
        infoFound.textContent = foundPairs;
        if (!ctx.examMode) ctx.cheer();
        firstCard = null;
        if (foundPairs === pairs.length && !answered) {
          answered = true;
          const seconds = Math.max(1, Math.round((Date.now() - startTs) / 1000));
          ctx.recordAnswer(pairs.length, pairs.length, [
            'Movimientos: ' + moves,
            'Tiempo: ' + seconds + 's',
          ]);
        }
  ...
  // =====================================================================
  // COMICDIALOGUE (cómic / chat con burbujas a completar)
  // =====================================================================
  reg('comicdialogue', function (ctx) {
    const panels = ctx.data || [];

    const wrap = document.createElement('div');

    const bankEl = document.createElement('div');
    bankEl.className = 'cd-bank';
    bankEl.id = 'cd-bank-' + ctx.exerciseId;
    bankEl.style.cssText = 'display:flex;flex-wrap:wrap;gap:8px;padding:14px;background:#FFF8E7;border:3px dashed #1a1a1a;border-radius:12px;margin-bottom:16px;min-height:50px;box-shadow:2px 2px 0 rgba(0,0,0,0.1);';
    wrap.appendChild(bankEl);

    const allAnswers = [];
    panels.forEach(panel => {
      (panel.bubbles || []).forEach(b => { if (b.blank && b.answer) allAnswers.push(String(b.answer)); });
      (panel.bank || []).forEach(w => { if (w) allAnswers.push(String(w)); });
    });
    const banked = C.shuffle(allAnswers.slice());

    panels.forEach((panel, pIdx) => {
      const panelEl = document.createElement('div');
      panelEl.className = 'cd-panel';
      panelEl.style.cssText = 'background:#fff;border:4px solid #1a1a1a;border-radius:14px;padding:14px 14px 12px;margin-bottom:18px;box-shadow:4px 4px 0 #1a1a1a;position:relative;overflow:hidden;';
      const dots = document.createElement('div');
      dots.style.cssText = 'position:absolute;top:6px;right:10px;width:54px;height:24px;background-image:radial-gradient(circle,#FF6B9D 1.5px,transparent 2px);background-size:8px 8px;opacity:0.55;pointer-events:none;';
      panelEl.appendChild(dots);

      if (panel.title) {
        const t = document.createElement('div');
        t.style.cssText = 'background:#FFD700;border:3px solid #1a1a1a;border-radius:10px;padding:5px 14px;font-weight:900;text-transform:uppercase;letter-spacing:1px;display:inline-block;margin:-26px 0 12px -4px;transform:rotate(-1.5deg);box-shadow:2px 2px 0 #1a1a1a;position:relative;z-index:1;';
      ...
      (panel.bubbles || []).forEach((b, bIdx) => {
        const row = document.createElement('div');
        const isA = b.author !== 'B';
        row.style.cssText = 'display:flex;justify-content:' + (isA ? 'flex-start' : 'flex-end') + ';margin:8px 0;gap:8px;align-items:flex-end;';

        const avatar = document.createElement('div');
        avatar.style.cssText = 'flex-shrink:0;width:38px;height:38px;border:3px solid #1a1a1a;border-radius:50%;background:' + (isA ? '#4FC3F7' : '#FF6B9D') + ';color:#fff;font-weight:900;display:grid;place-items:center;font-size:16px;text-shadow:1px 1px 0 #1a1a1a;box-shadow:2px 2px 0 #1a1a1a;';
        avatar.textContent = b.author === 'B' ? 'B' : 'A';

        const bubble = document.createElement('div');
        bubble.style.cssText = 'position:relative;max-width:75%;padding:10px 14px;border:3px solid #1a1a1a;border-radius:18px;font-weight:bold;font-size:15px;line-height:1.35;background:' + (isA ? '#FFFEF0' : '#E1F5FE') + ';box-shadow:2px 2px 0 #1a1a1a;';

        if (b.blank) {
          const slot = document.createElement('span');
          slot.className = 'cd-slot';
          slot.dataset.answer = b.answer || '';
          slot.dataset.pidx = pIdx;
          slot.dataset.bidx = bIdx;
          slot.style.cssText = 'display:inline-block;min-width:120px;min-height:24px;padding:3px 10px;background:#fff;border:3px dashed #1a1a1a;border-radius:10px;vertical-align:middle;';
          bubble.appendChild(slot);
        } else {
          bubble.appendChild(document.createTextNode(b.text || ''));
        }

        if (isA) { row.appendChild(avatar); row.appendChild(bubble); }
    ...
    banked.forEach(w => {
      ...
      chip.textContent = w;
      chip.style.cssText = 'background:#FFD700;color:#1a1a1a;border:3px solid #1a1a1a;border-radius:10px;padding:6px 12px;font-weight:900;cursor:grab;box-shadow:2px 2px 0 #1a1a1a;user-select:none;-webkit-user-select:none;';
      attachDrag(chip);
    ...
    function attachDrag(el) {
      C.makeDraggable(el, {
        onMove: function (ev) {
          ...
          document.querySelectorAll('.cd-slot').forEach(s => s.classList.remove('civica-drop-hover'));
          const slot = under && under.closest && under.closest('.cd-slot');
          if (slot) slot.classList.add('civica-drop-hover');
        },
        onDrop: function (ev) {
          const under = ev.under;
          ...
          const slot = under && under.closest && under.closest('.cd-slot');
          if (slot) {
            const existing = slot.querySelector('.cd-chip');
            if (existing) bankEl.appendChild(existing);
            slot.appendChild(el);
            if (!ctx.examMode) {
              if (C.normalize(el.textContent) === C.normalize(slot.dataset.answer)) ctx.cheer();
              else ctx.comfort();
            }
            check();
          } else {
            const parent = el.parentElement;
            if (!parent || !parent.classList.contains('cd-slot')) {
              bankEl.appendChild(el);
            }
          }
        }
    ...
    function check() {
      const slots = wrap.querySelectorAll('.cd-slot');
      let correct = 0;
      const details = [];
      slots.forEach(slot => {
        const chip = slot.querySelector('.cd-chip');
        const ans = slot.dataset.answer;
        if (chip && C.normalize(chip.textContent) === C.normalize(ans)) {
          correct++;
          slot.style.borderColor = '#4CAF50';
          slot.style.background = '#E8F5E9';
          details.push(ans + ': OK');
        } else {
          slot.style.borderColor = chip ? '#E63946' : '#1a1a1a';
          slot.style.background = chip ? '#FFEBEE' : '#fff';
          details.push(ans + ': ' + (chip ? chip.textContent + ' (X)' : 'vacío'));
        }
      });
      ctx.recordAnswer(correct, slots.length || 1, details);
      if (!ctx.examMode && slots.length > 0 && correct === slots.length) ctx.cheer();
    }
  ...
  // =====================================================================
  // 15. CV_TEXTANSWER (respuesta abierta de texto)
  // =====================================================================
  ...
  // correcta si el alumno escribió al menos 16 caracteres.
  //
  // data: [
  //   { q: '¿Qué entiendes por dignidad humana?', rows: 3 },
  ...
  // alcanzar 16 caracteres. `minLength` se conserva solo por compatibilidad.
  reg('cv_textanswer', function (ctx) {
    ...
    items.forEach((it, i) => {
      ...
      row.className = 'civica-panel';
      row.style.cssText = 'margin:14px 0;';
      const minLen = CIVICA_OPEN_ANSWER_MIN_LENGTH;
      const rows = it.rows || 3;
      const placeholder = it.placeholder || 'Escribe tu respuesta aquí...';

      row.innerHTML = `
        <div style="font-weight:600; margin-bottom:10px; font-size:15px; color:var(--civica-text);">
          ${i + 1}. ${C.escapeHTML(it.q)}
        </div>
        <textarea class="civica-input civica-input-allow-copy cv-ta-input"
                  data-i="${i}"
                  rows="${rows}"
                  placeholder="${C.escapeHTML(placeholder)}"></textarea>
        <div class="cv-ta-meta" data-meta="${i}"
             style="margin-top:6px; font-size:12px; color:var(--civica-text-muted); display:flex; justify-content:space-between;">
          <span class="cv-ta-counter">0 caracteres</span>
          <span class="cv-ta-min">Mínimo: ${minLen} caracteres</span>
        </div>
      `;
      wrap.appendChild(row);
      const ta = row.querySelector('.cv-ta-input');
      const counter = row.querySelector('.cv-ta-counter');
      ta.addEventListener('input', () => {
        const len = ta.value.trim().length;
        counter.textContent = len + ' caracteres';
        counter.style.color = len >= minLen ? 'var(--civica-success)' : 'var(--civica-text-muted)';
      });
      inputs.push({ ta, item: it });
    });
    ...
    wrap.appendChild(gradeButton(() => {
      ...
      inputs.forEach((slot, i) => {
        ...
        const ok = isDevelopedOpenAnswer(text);

        if (ok) correct++;
        if (!ctx.examMode) {
          slot.ta.classList.toggle('correct', ok);
          slot.ta.classList.toggle('wrong', !ok);
          slot.ta.disabled = true;
        } else {
          slot.ta.disabled = true;
        }

        const preview = text ? (text.length > 60 ? text.substring(0, 60) + '...' : text) : '(vacío)';
        details.push((i + 1) + '. ' + (ok ? 'OK' : 'X') + ' - ' + preview);
      });

      ctx.recordAnswer(correct, items.length, details, userAnswer);
      if (!ctx.examMode) {
        if (correct === items.length) ctx.cheer(); else ctx.comfort();
      }
    }, ctx));
  ...
  // =====================================================================
  // 16. CV_TABLE (tabla NxN configurable)
  // =====================================================================
  // Tabla rectangular (N filas x M columnas) donde cada celda puede ser:
  //   - { type: 'text', value: '...' }                          → solo lectura
  //   - { type: 'header', value: '...' }                        → cabecera <th>
  //   - { type: 'image', src: '...' }                           → imagen
  //   - { type: 'input', placeholder: '...' }                 → respuesta abierta multilínea (16+ caracteres)
  //   - { type: 'select', options:['...','...'], answer:1 }     → dropdown
  //   - { type: 'drop', answer: 'token-id' }                    → recibe un token del banco
  //   - { type: 'check', answer: true|false }                   → celda click cycle: '' → ✓ → ✗ → ''
  //
  // data: {
  //   columns: ['Encabezado 1', 'Encabezado 2', ...],   // opcional, se usa como fila <th>
  //   rows: [
  //     [ {type:...}, {type:...}, ... ],   // fila 1
  //     [ {type:...}, {type:...}, ... ],   // fila 2
  //   ],
  //   bank: ['opcion 1', 'opcion 2', ...]  // opcional, solo si hay celdas tipo 'drop'
  // }
  reg('cv_table', function (ctx) {
    const data = ctx.data || {};
    const columns = data.columns || null;
    const rows = data.rows || [];
    const bankItems = Array.isArray(data.bank) ? data.bank.slice() : [];

    const wrap = document.createElement('div');

    // Banco de tokens (si hay celdas drop)
    let bankEl = null;
    const hasDrop = rows.some(r => r.some(c => c && c.type === 'drop'));
    if (hasDrop && bankItems.length) {
      bankEl = document.createElement('div');
      bankEl.className = 'civica-bank';
      bankEl.innerHTML = '<div class="civica-bank-empty-msg" style="display:none">Banco vacío</div>';

      C.shuffle(bankItems).forEach((label, i) => {
        const tk = document.createElement('div');
        tk.className = 'civica-token';
        tk.textContent = label;
        tk.dataset.tokenLabel = label;
        bankEl.appendChild(tk);
        attachTokenDrag(tk);
      });
      wrap.appendChild(bankEl);
    }
    ...
    tWrap.className = 'civica-table-wrap';
    ...
    const cellRefs = []; // array paralelo a rows: { td, cell, value/state }

    rows.forEach((rowDef, rIdx) => {
      const tr = document.createElement('tr');
      const rowRefs = [];
      rowDef.forEach((cellDef, cIdx) => {
        const cell = cellDef || { type: 'text', value: '' };
        let td;
        if (cell.type === 'header') {
          td = document.createElement('th');
          td.innerHTML = C.escapeHTML(cell.value || '');
        } else {
          td = document.createElement('td');
          buildCellContent(td, cell, rIdx, cIdx);
        }
        tr.appendChild(td);
        rowRefs.push({ td, cell });
      });
    ...
    function buildCellContent(td, cell, rIdx, cIdx) {
      if (cell.type === 'text') {
        td.textContent = cell.value || '';
      } else if (cell.type === 'image') {
        td.className = 'civica-cell-image';
        td.innerHTML = `<img src="${C.escapeHTML(cell.src)}" alt="${C.escapeHTML(cell.alt || '')}">`;
      } else if (cell.type === 'input') {
        td.className = 'civica-cell-input';
        td.innerHTML = `<textarea class="civica-input-allow-copy civica-table-textarea"
                        data-r="${rIdx}" data-c="${cIdx}"
                        rows="3"
                        placeholder="${C.escapeHTML(cell.placeholder || '')}"
                        autocomplete="off"></textarea>`;
        const ta = td.querySelector('.civica-table-textarea');
        const autoGrow = () => {
        ...
        td.className = 'civica-cell-select';
        const opts = (cell.options || [])
          .map((o, i) => `<option value="${i}">${C.escapeHTML(o)}</option>`).join('');
        td.innerHTML = `<select data-r="${rIdx}" data-c="${cIdx}">
          <option value="">— Selecciona —</option>${opts}
        </select>`;
      } else if (cell.type === 'drop') {
        td.className = 'civica-cell-drop';
        td.dataset.r = rIdx;
        td.dataset.c = cIdx;
        td.dataset.placeholder = cell.placeholder || 'Suelta aquí';
        td.textContent = td.dataset.placeholder;
      } else if (cell.type === 'check') {
        td.className = 'civica-cell-checkable';
        td.dataset.r = rIdx;
        td.dataset.c = cIdx;
        td.dataset.state = '';
        td.textContent = '—';
        td.addEventListener('click', () => {
          ...
          const cur = td.dataset.state;
          // ciclo: '' -> check -> x -> ''
          let next = '';
          if (cur === '') next = 'check';
          else if (cur === 'check') next = 'x';
          else next = '';
          applyCheckState(td, next);
        });
      } else {
        td.textContent = cell.value || '';
      }
    ...
    function applyCheckState(td, state) {
      td.dataset.state = state;
      td.classList.remove('is-check', 'is-x');
      if (state === 'check') {
        td.textContent = '✓';
        td.classList.add('is-check');
      } else if (state === 'x') {
        td.textContent = '✗';
        td.classList.add('is-x');
      } else {
        td.textContent = '—';
      }
    }

    // Drag & Drop para tokens
    function attachTokenDrag(token) {
      C.makeDraggable(token, {
        onMove({ under }) {
          // Limpiar highlight de drops anteriores
          wrap.querySelectorAll('.civica-cell-drop').forEach(d => {
            d.style.outline = '';
          });
          const cell = under && under.closest && under.closest('.civica-cell-drop');
          if (cell && cell.dataset.locked !== '1') {
            cell.style.outline = '3px solid var(--civica-explore)';
        ...
        onDrop({ under }) {
          ...
          // Si la celda ya tiene un token, devolverlo al banco
          const existingLabel = cell.dataset.tokenLabel;
          if (existingLabel) {
            returnTokenToBank(existingLabel);
          }
          cell.classList.add('has-token');
          cell.dataset.tokenLabel = token.dataset.tokenLabel;
          cell.textContent = token.dataset.tokenLabel;
          // Remover token del banco
          token.remove();
          updateBankEmptyMsg();
        }
      });
    ...
    function returnTokenToBank(label) {
      if (!bankEl) return;
      const tk = document.createElement('div');
      tk.className = 'civica-token';
      tk.textContent = label;
    ...
    function updateBankEmptyMsg() {
      if (!bankEl) return;
      const tokens = bankEl.querySelectorAll('.civica-token');
      const msg = bankEl.querySelector('.civica-bank-empty-msg');
      if (msg) msg.style.display = tokens.length === 0 ? 'block' : 'none';
    }

    // Click en una celda drop con token: regresa el token al banco
    if (bankEl) {
      table.addEventListener('click', e => {
        const cell = e.target.closest('.civica-cell-drop');
        if (!cell) return;
        if (cell.dataset.locked === '1') return;
        if (!cell.dataset.tokenLabel) return;
        const label = cell.dataset.tokenLabel;
        cell.classList.remove('has-token');
        cell.textContent = cell.dataset.placeholder || 'Suelta aquí';
        delete cell.dataset.tokenLabel;
        returnTokenToBank(label);
      });
    }
    ...
    wrap.appendChild(gradeButton(() => {
      let correct = 0, total = 0;
      const details = [];
      const userAnswer = { rows: [] };

      cellRefs.forEach((rowRefs, rIdx) => {
        const rowSnap = [];
        rowRefs.forEach(({ td, cell }, cIdx) => {
          if (!cell || cell.type === 'text' || cell.type === 'image' || cell.type === 'header') {
            rowSnap.push(null);
            return;
          }
          ...
          if (cell.type === 'input') {
            total++;
            const inp = td.querySelector('textarea, input');
            const val = (inp.value || '').trim();
            const ok = isDevelopedOpenAnswer(val);
            if (ok) correct++;
            if (!ctx.examMode) {
              inp.classList.toggle('correct', ok);
              inp.classList.toggle('wrong', !ok);
            ...
            inp.disabled = true;
            details.push(`F${rIdx+1}C${cIdx+1}: ${ok ? 'OK' : 'X (mínimo '+CIVICA_OPEN_ANSWER_MIN_LENGTH+' caracteres; escritos: '+val.length+')'}`);
            rowSnap.push(val);
            ...
            const val = sel.value;
            const ok = String(val) === String(cell.answer);
            if (ok) correct++;
            if (!ctx.examMode) {
              sel.classList.toggle('correct', ok);
            ...
            sel.disabled = true;
            const opts = cell.options || [];
            const chosen = val !== '' ? opts[+val] : 'vacio';
            const expected = opts[cell.answer];
            details.push(`F${rIdx+1}C${cIdx+1}: ${ok ? 'OK' : 'X (esperado: '+expected+', tuyo: '+chosen+')'}`);
            rowSnap.push(chosen);

          } else if (cell.type === 'drop') {
            total++;
            const placed = td.dataset.tokenLabel || '';
            const ok = C.normalize(placed) === C.normalize(cell.answer || '');
            if (ok) correct++;
            if (!ctx.examMode) {
              td.style.background = ok ? 'var(--civica-deepen-soft)' : '#FEE2E2';
              td.style.color = ok ? '#15803D' : '#B91C1C';
            }
            td.dataset.locked = '1';
            details.push(`F${rIdx+1}C${cIdx+1}: ${ok ? 'OK' : 'X (esperado: '+cell.answer+', tuyo: '+(placed||'vacio')+')'}`);
            rowSnap.push(placed);

          } else if (cell.type === 'check') {
            total++;
            const state = td.dataset.state || '';
            const userBool = state === 'check' ? true : (state === 'x' ? false : null);
            const ok = userBool !== null && userBool === !!cell.answer;
            if (ok) correct++;
            if (!ctx.examMode && userBool !== null) {
              // ya tiene clase is-check / is-x del estado; añadimos borde según resultado
              td.style.boxShadow = ok
                ? 'inset 0 0 0 3px var(--civica-success)'
                : 'inset 0 0 0 3px var(--civica-error)';
            }
            td.dataset.locked = '1';
            details.push(`F${rIdx+1}C${cIdx+1}: ${ok ? 'OK' : 'X (esperado: '+(cell.answer?'✓':'✗')+', tuyo: '+(userBool===null?'sin marcar':(userBool?'✓':'✗'))+')'}`);
            rowSnap.push(state);
        ...
        userAnswer.rows.push(rowSnap);
      });

      if (total === 0) total = 1; // evita división por cero si no hay celdas evaluables
      ctx.recordAnswer(correct, total, details, userAnswer);
      if (!ctx.examMode) {
        if (correct === total) ctx.cheer(); else ctx.comfort();
      }
    }, ctx));
