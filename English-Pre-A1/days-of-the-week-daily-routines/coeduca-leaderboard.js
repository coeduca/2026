...
(function (global) {
  ...
  const SUPPORTED_GAMES = new Set(['snake', 'dino', 'pills', 'hangman']);
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .rank-1 .cg-leaderboard-rank::before {
        ...
        background:
          radial-gradient(circle at 21% 72%, #e84b55 0 1.5px, transparent 2px),
          radial-gradient(circle at 50% 72%, #36a9e1 0 1.5px, transparent 2px),
          radial-gradient(circle at 79% 72%, #e84b55 0 1.5px, transparent 2px),
          linear-gradient(135deg, #fff4a3 0%, #ffc928 42%, #d98b00 100%);
        clip-path: polygon(0 20%, 24% 45%, 39% 0, 57% 44%, 79% 21%, 97% 53%, 91% 100%, 10% 100%);
  ...
  function createList(title, rows, limit) {
  ...
  function create(ctx, game, minimumScore) {
    ...
    let bestSent = Number(localStorage.getItem(`coeduca-ranking-${game}-${student.nie}`)) || 0;
    ...
    async function refresh() {
      try {
        ...
        grids.replaceChildren(
          createList('🌎 Top 10 global', data && data.global, 10),
          createList(`🎓 Top 5 · ${student.grade || 'mi grado'}`, data && data.grade, 5)
        );
        status.textContent = '';
      } catch (error) {
